---
title: Remote Access
description: Fix tunnels, remote instances, and reaching ZedCode from another device.
---

# Remote Access

When you can't reach ZedCode from your phone or another machine, the fix depends on how you're connecting.
...
- confirm the server is running with `zedcode status`
...
- for **Anywhere** pairing, check **Settings → Remote Instances → ZedCode Relay** on the server: it should say **Connected**; if not, disable and re-enable it
...
When a [remote instance](/remote-instances/) stalls, ZedCode names the step that failed:

- **auth** — your SSH or UI password was rejected; re-enter it
- **install / start** — ZedCode couldn't set up or start the server on the remote machine; check that machine's requirements
- **forwarding** — the connection is up but the port isn't reaching you; try a different local port

## Behind your own server

If you put ZedCode behind a reverse proxy and it loads oddly or won't connect, see [Reverse Proxy](/reverse-proxy/).
