---
title: Dostęp zdalny
description: Napraw tunele, zdalne instancje i dostęp do ZedCode z innego urządzenia.
---

# Dostęp zdalny

Gdy nie możesz dotrzeć do ZedCode z telefonu lub innej maszyny, rozwiązanie zależy od tego, jak się łączysz.
...
- potwierdź, że serwer działa, za pomocą `zedcode status`
...
- przy parowaniu **Wszędzie** sprawdź **Settings → Remote Instances → ZedCode Relay** na serwerze: status powinien brzmieć **Połączono**; jeśli nie, wyłącz i włącz relay ponownie
...
- uruchom `zedcode tunnel status --all`
...
Gdy [zdalna instancja](/pl/remote-instances/) się zatnie, ZedCode wskazuje krok, który zawiódł:

- **auth** — Twoje hasło SSH lub UI zostało odrzucone; wpisz je ponownie
- **install / start** — ZedCode nie mógł skonfigurować lub uruchomić serwera na zdalnej maszynie; sprawdź wymagania tej maszyny
- **forwarding** — połączenie działa, ale port do Ciebie nie dociera; spróbuj innego portu lokalnego

## Za własnym serwerem

Jeśli umieściłeś ZedCode za reverse proxy i ładuje się dziwnie lub nie chce się połączyć, zobacz [Reverse Proxy](/pl/reverse-proxy/).
