---
title: Reverse proxy
description: Skonfiguruj ZedCode poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz ZedCode za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że ZedCode działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli ZedCode już kompresuje odpowiedzi.
...
- ZedCode dostępny bezpośrednio w LAN
...
- ZedCode kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez ZedCode, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
