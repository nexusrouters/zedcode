---
title: Proxy reverso
description: Configure o ZedCode corretamente atrás do Nginx, Nginx Proxy Manager ou outro proxy reverso.
---

# Proxy reverso

Use esta página se você executa o ZedCode atrás do Nginx, Nginx Proxy Manager, Caddy, Cloudflare ou outro proxy reverso.

## Antes de colocar um proxy

1. Primeiro confirme que o ZedCode funciona diretamente.
...
- Desative o gzip no proxy se o ZedCode já comprime as respostas.
...
- ZedCode acessível diretamente na LAN
...
- O ZedCode comprime as respostas HTTP com gzip (limite de 1 KB).
...
As rotas de streaming SSE são excluídas da compressão pelo ZedCode, mas a CDN ainda pode armazená-las em buffer. Consulte a documentação da sua CDN sobre como desativar o buffer nas rotas SSE.
