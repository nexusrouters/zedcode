...
This package is docs content source-of-truth for ZedCode.
...
- Lead with the task, not background. The first line should say what the page is
  for ("Use `zedcode tunnel` to expose a running ZedCode instance.").
...
- A basic page should fit in a screen or two. Long, dense reference pages (like
  ...
  this page if you run ZedCode behind...").
...
```mdx
3. Run `zedcode --ui-password be-creative-here`.
4. Open the printed URL (usually `http://localhost:3000`).

You should land on the ZedCode session list. If you see it, the server is
running.
```
...
- Explain a term the first time it appears, in parentheses, in everyday words:
  - good: start a tunnel (a public link to your local ZedCode)
...
2. Add frontmatter at top:
   ...
   ---
   title: Remote Access
   description: Access ZedCode from outside your local network.
   ---
   ```
...
The docs are translated into the same languages the ZedCode app ships in.
...
`zedcode-website/apps/docs/astro.config.mjs` (`locales`). If a language is
...
- brand and product nouns: ZedCode, OpenCode, VS Code, PWA, GitHub, Discord,
  macOS, SSH
...
## Sync into zedcode-website

`zedcode-website` renders/deploys docs via Starlight in `apps/docs`.

After docs content updates here:

1. copy `packages/docs/content/docs/*` -> `zedcode-website/apps/docs/src/content/docs/*`
   (this is recursive, so locale folders like `uk/` and `zh-cn/` carry over with
   no extra steps)
2. map `packages/docs/sidebar.config.json` into `zedcode-website/apps/docs/astro.config.mjs` sidebar
