...
Rendering and hosting still happen in `zedcode-website` (`apps/docs`).
...
- creates `zedcode-docs-source-<sha>.tar.gz`
...
to `zedcode-website` so it re-syncs and redeploys the docs. This fires on
...
- `ZEDCODE_WEBSITE_REPO_TOKEN` — a token with `contents: write` (classic
  `repo` scope, or fine-grained with Contents: read & write) on
  `zedcode/zedcode-website`. **Without it the dispatch step is skipped**
...
- `source_repo`
- `source_ref` — the ref the website checks out from `zedcode` (`main` on a
  push, the tag on a release)
- `archive_name`

`zedcode-website`'s `deploy-docs.yml` listens for this event
(`repository_dispatch: types: [docs_source_updated]`), checks out
`zedcode` at `source_ref`, runs `docs:sync`, builds `apps/docs`, and deploys
to Cloudflare Pages. That repo needs its own secrets: `ZEDCODE_REPO_TOKEN`
