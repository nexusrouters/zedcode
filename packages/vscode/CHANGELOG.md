...
- Permissions: per-session auto-accept works again, persists across extension restarts, and applies to subagent sessions while an ZedCode view is open.
...
- Chat: sessions with an active [goal](https://docs.zedcode.dev/session-goals/) (started from the web or desktop app) show the goal strip with its live status above the composer.
