...
![VS Code Extension](https://github.com/zedcode/zedcode/raw/HEAD/packages/vscode/extension.jpg)

**Like the extension? There's also a [desktop app and web version](https://github.com/zedcode/zedcode) with even more features.**
...
Plus everything from the shared ZedCode UI: branchable timeline, smart tool UIs, voice mode, Git workflows, and more.

## Commands

| Command | Description |
|---------|-------------|
| `ZedCode: Focus Chat` | Focus the chat panel |
| `ZedCode: New Session` | Start a new chat session |
| `ZedCode: Open Sidebar` | Open the ZedCode sidebar |
| `ZedCode: Open Agent Manager` | Launch parallel multi-model runs |
| `ZedCode: Open Session in Editor` | Open current or new session in an editor tab |
| `ZedCode: Settings` | Open extension settings |
| `ZedCode: Restart API Connection` | Restart the OpenCode API process |
| `ZedCode: Show OpenCode Status` | Debug info for development or bug reports |

### Right-click menu

Select code in the editor, right-click, and find the **ZedCode** submenu:
...
|---------|---------|-------------|
| `zedcode.apiUrl` | _(empty)_ | URL of an external OpenCode API server. Leave empty to auto-start a local instance. |
| `zedcode.opencodeBinary` | _(empty)_ | Absolute path to the `opencode` CLI binary. Useful when PATH lookup fails. Requires window reload to apply. |
...
- `ZEDCODE_VSCODE_BIN=cursor bun run vscode:dev`
- `ZEDCODE_VSCODE_DEV_WORKSPACE=/path/to/workspace bun run vscode:dev`
...
Install locally: `code --install-extension packages/vscode/zedcode-*.vsix`
