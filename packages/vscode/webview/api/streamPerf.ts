...
declare global {
  interface Window {
    __zedcodeVsCodeStreamPerfState__?: StreamPerfState;
  }
}
...
const ensurePerfState = (): StreamPerfState | null => {
  ...
  if (!window.__zedcodeVsCodeStreamPerfState__) {
    const startedAt = Date.now();
    window.__zedcodeVsCodeStreamPerfState__ = {
  ...
  return window.__zedcodeVsCodeStreamPerfState__;
};
