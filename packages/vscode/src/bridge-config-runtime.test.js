...
describe('VS Code config bridge plugin parity', () => {
  test('explicit config reload restarts OpenCode', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zedcode-vscode-reload-'));
    tempRoots.push(root);
  ...
  test('removes agent fields when update payload sends null', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zedcode-vscode-agent-null-'));
    tempRoots.push(root);
    const ctx = createCtx(root);
  ...
  test('creates, lists, updates, and deletes project plugin entries', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zedcode-vscode-plugins-'));
    tempRoots.push(root);
    const ctx = createCtx(root);
  ...
  test('creates and reads project plugin files', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zedcode-vscode-plugin-files-'));
    tempRoots.push(root);
    const ctx = createCtx(root);
  ...
  test('updates and deletes user plugin entries from OPENCODE_CONFIG source', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zedcode-vscode-custom-config-'));
    tempRoots.push(root);
  ...
  test('writes user plugin files next to OPENCODE_CONFIG', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zedcode-vscode-custom-files-'));
    tempRoots.push(root);
    const configDir = path.join(root, 'custom-config');
    const configPath = path.join(configDir, 'opencode.json');
    fs.mkdirSync(configDir, { recursive: true });
  ...
  test('creates MCP config with deferred restart when restart would fail', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zedcode-vscode-mcp-deferred-'));
    tempRoots.push(root);
