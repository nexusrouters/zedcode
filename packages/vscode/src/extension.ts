...
const SETTINGS_KEY = 'zedcode.settings';
...
export async function activate(context: vscode.ExtensionContext) {
  outputChannel = vscode.window.createOutputChannel('ZedCode');
  ...
  const attemptMoveChatToRightSidebar = async (): Promise<'moved' | 'unsupported' | 'failed'> => {
    ...
    try {
      await vscode.commands.executeCommand('zedcode.chatView.focus');
      await vscode.commands.executeCommand(moveCommandId);
      return 'moved';
    } catch (error) {
      outputChannel?.appendLine(
        `[ZedCode] Failed moving chat view to right sidebar (command=${moveCommandId}): ${error instanceof Error ? error.message : String(error)}`
      );
      return 'failed';
    }
  };
  ...
  const maybeMoveChatToRightSidebarOnStartup = async () => {
    ...
    const attempted = context.globalState.get<boolean>('zedcode.sidebarAutoMoveAttempted') || false;
    if (attempted) return;
    await context.globalState.update('zedcode.sidebarAutoMoveAttempted', true);
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.openSidebar', async () => {
      // Best-effort: open the container (if available), then focus the chat view.
      try {
        await vscode.commands.executeCommand('workbench.view.extension.zedcode');
      } catch (e) {
        outputChannel?.appendLine(`[ZedCode] workbench.view.extension.zedcode failed: ${e}`);
      }

      try {
        await vscode.commands.executeCommand('zedcode.chatView.focus');
      } catch (e) {
        outputChannel?.appendLine(`[ZedCode] zedcode.chatView.focus failed: ${e}`);
        vscode.window.showErrorMessage(t('ZedCode: Failed to open sidebar - {0}', String(e)));
        return false;
      }

      if (!chatViewProvider?.hasResolvedView()) {
        outputChannel?.appendLine('[ZedCode] Chat sidebar focus completed before the webview was resolved');
        vscode.window.showWarningMessage(t('ZedCode: Chat sidebar is not ready'));
        return false;
      }
  ...
  const revealChatViewForPayload = async () => {
    const opened = await vscode.commands.executeCommand<boolean>('zedcode.openSidebar');
    if (!opened) {
      return false;
    }
    ...
    if (!chatViewProvider?.hasResolvedView()) {
      outputChannel?.appendLine('[ZedCode] Chat sidebar webview was disposed before payload delivery');
      vscode.window.showWarningMessage(t('ZedCode: Chat sidebar is not ready'));
      return false;
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.focusChat', async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.internal.settingsSynced', (settings: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.internal.permissionAutoAcceptSynced', (snapshot: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.openAgentManager', () => {
      agentManagerProvider?.createOrShow();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.setActiveSession', (sessionId: unknown, title?: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.openActiveSessionInEditor', () => {
      if (!activeSessionId) {
        vscode.window.showInformationMessage(t('ZedCode: No active session'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.openSessionInEditor', (sessionId: string, title?: string) => {
      if (typeof sessionId !== 'string' || sessionId.trim().length === 0) {
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.openNewSessionInEditor', () => {
      sessionEditorProvider?.createOrShowNewSession();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.openCurrentOrNewSessionInEditor', () => {
      if (activeSessionId) {
        sessionEditorProvider?.createOrShow(activeSessionId, activeSessionTitle ?? undefined);
      } else {
        sessionEditorProvider?.createOrShowNewSession();
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.restartApi', async () => {
      try {
        ...
        vscode.window.showInformationMessage(t('ZedCode: API connection restarted'));
      } catch (e) {
        vscode.window.showErrorMessage(t('ZedCode: Failed to restart API - {0}', String(e)));
      }
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.addToContext', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('ZedCode [Add to Context]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('ZedCode [Add to Context]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.attachExplorerToChat', async (resource?: vscode.Uri, resources?: vscode.Uri[]) => {
      ...
      if (attachedFiles.length === 0) {
        vscode.window.showWarningMessage(t('ZedCode: No file selected to mention'));
        return;
      }
      ...
      if (skippedEntries.length > 0) {
        vscode.window.showInformationMessage(t('ZedCode: Some selected entries were skipped (folders or unsupported resources)'));
      }
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.explain', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('ZedCode [Explain]: No active editor'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.improveCode', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('ZedCode [Improve Code]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('ZedCode [Improve Code]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.newSession', async (directory?: unknown) => {
      ...
      if (!folderPath && candidates.length === 0) {
        vscode.window.showInformationMessage('ZedCode: No folder is open. Open a folder to start a new session.');
        return;
      }
      ...
      if (openCodeManager) {
        ...
        if (!result.success) {
          vscode.window.showErrorMessage(`ZedCode: ${result.error}`);
          return;
        }
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.showSettings', () => {
      chatViewProvider?.showSettings();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('zedcode.showOpenCodeStatus', async () => {
      const config = vscode.workspace.getConfiguration('zedcode');
      ...
      const lines = [
        ...
        `ZedCode version: ${extensionVersion || '(unknown)'}`,
        ...
        `OpenCode binary (configured): ${(vscode.workspace.getConfiguration('zedcode').get<string>('opencodeBinary') || '').trim() || '(none)'}`,
