type UpgradeCapability = {
  supported: boolean;
  manager: 'opencode' | 'external' | 'zedcode' | null;
...
const getCapability = (manager?: OpenCodeUpgradeManager): UpgradeCapability => {
  if (isWindowsArm64()) return { supported: false, manager: 'zedcode', reason: 'windows-arm64-workaround' };
...
export const upgradeManagedOpenCode = async (manager: OpenCodeUpgradeManager | undefined, target?: unknown): Promise<UpgradeResult> => {
  ...
  if (!upgrade.supported || !apiUrl || !manager) {
    return { status: 409, body: { success: false, code: 'OPENCODE_UPGRADE_UNSUPPORTED', error: 'This OpenCode runtime cannot be upgraded by ZedCode.' } };
  }
