...
import { mintOutsideFileGrant } from '@zedcode/web/server/lib/fs/routes.js';

const execFileAsync = promisify(execFile);

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const isDev = process.env.ZEDCODE_ELECTRON_DEV === '1' || !app.isPackaged;
const electronStartupStartedAt = performance.now();

const DEEP_LINK_PROTOCOL = 'zedcode';
const UI_PROTOCOL = 'zedcode-ui';
const PACKAGED_APP_USER_MODEL_ID = 'dev.zedcode.desktop';
const DEV_APP_USER_MODEL_ID = 'dev.zedcode.desktop.dev';
...
// ~/Library/Logs/ZedCode/ (not ~/Library/Logs/@zedcode/electron/).
app.setName('ZedCode');
if (process.platform === 'linux') {
  app.setDesktopName('zedcode.desktop');
}
if (isDev) {
  app.setPath('userData', path.join(app.getPath('appData'), 'ZedCode Dev'));
}
...
if (shouldIgnoreLoopbackConnectionLimit({
  development: isDev,
  packagedUi: process.env.ZEDCODE_ELECTRON_USE_BUNDLED_UI === '1',
})) {
...
// that output never lands in ~/Library/Logs/ZedCode/main.log and we
...
const recordElectronStartupPerformance = (phase, details = {}) => {
  const enabled = STARTUP_PERF_ENABLED_VALUES.has(String(process.env.ZEDCODE_STARTUP_PERF ?? '').toLowerCase());
...
const readAppMetadata = () => {
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (parsed?.name === '@zedcode/electron' && typeof parsed.version === 'string') {
        return { name: parsed.name, version: parsed.version };
      }
    } catch {
    }
  }
  return { name: '@zedcode/electron', version: app.getVersion() };
};
...
const GITHUB_BUG_REPORT_URL = 'https://github.com/zedcode/zedcode/issues/new?template=bug_report.yml';
const GITHUB_FEATURE_REQUEST_URL = 'https://github.com/zedcode/zedcode/issues/new?template=feature_request.yml';
...
const quitConfirmationMessage = () => {
  ...
  return `ZedCode detected ${reasons.join(', ')}. Quitting now will stop sidecar/background processes and may interrupt pending work.`;
};
...
const refreshQuitRiskFlags = async () => {
  ...
  const scheduledUrl = `${base}/api/zedcode/scheduled-tasks/status`;
  const tunnelUrl = `${base}/api/zedcode/tunnel/status`;
...
const settingsFilePath = () => {
  if (typeof process.env.ZEDCODE_DATA_DIR === 'string' && process.env.ZEDCODE_DATA_DIR.trim()) {
    return path.join(process.env.ZEDCODE_DATA_DIR.trim(), 'settings.json');
  }
  return path.join(os.homedir(), '.config', 'zedcode', 'settings.json');
};
...
// session expires) reuses the same "ZedCode Desktop" record instead of
...
const shouldUsePackagedUi = () => {
  if (process.env.ZEDCODE_ELECTRON_LOAD_SERVER_UI === '1') return false;
  if (process.env.ZEDCODE_ELECTRON_USE_BUNDLED_UI === '1') return true;
...
const injectRuntimeConfigIntoHtml = (html) => {
  ...
  const initScript = `<script>if(window.__ZEDCODE_LOCAL_ORIGIN__===undefined){window.__ZEDCODE_LOCAL_ORIGIN__=${JSON.stringify(localOrigin)};}if(window.__ZEDCODE_API_BASE_URL__===undefined){window.__ZEDCODE_API_BASE_URL__=${JSON.stringify(apiBaseUrl)};}if(window.__ZEDCODE_CLIENT_TOKEN__===undefined&&${JSON.stringify(state.clientToken || '')}){window.__ZEDCODE_CLIENT_TOKEN__=${JSON.stringify(state.clientToken || '')};}</script>`;
...
/**
 * The browser panel's own session, kept separate from ZedCode's.
 *
 ...
 * own storage.
 */
const BROWSER_PANEL_PARTITION = 'persist:zedcode-browser';
...
const maybeShowNativeNotification = (rawInput) => {
  ...
  notification.on('click', () => {
    ...
    if (sessionId) {
      emitToAllWindows('zedcode:open-session', { sessionId, directory });
    }
    release();
  });
...
import { pathLooksUserConfigured, mergePathValues } from '@zedcode/web/server/lib/opencode/path-utils.js';
import { clearAppImageArgv0FromProcessEnv } from '@zedcode/web/server/lib/inherited-env.js';
...
const shouldSkipLocalServer = () => {
  inheritUserShellEnv();
  return process.env.ZEDCODE_SKIP_LOCAL_SERVER === '1';
};
...
const spawnLocalServer = async () => {
  ...
  // The server module reads ENV_DESKTOP_NOTIFY / ZEDCODE_DIST_DIR /
  // ZEDCODE_RUNTIME at import time (top-level const), so these must be
  ...
  process.env.ZEDCODE_HOST = bindHost;
  process.env.ZEDCODE_DESKTOP_LAN_ACCESS_ACTIVE = effectiveLanAccessEnabled ? 'true' : 'false';
  if (lanAccessBlockedByMissingPassword) {
    process.env.ZEDCODE_DESKTOP_LAN_ACCESS_BLOCKED_REASON = 'missing-password';
  } else {
    delete process.env.ZEDCODE_DESKTOP_LAN_ACCESS_BLOCKED_REASON;
  }
  process.env.ZEDCODE_DIST_DIR = resolveWebDistDir();
  process.env.ZEDCODE_RUNTIME = 'desktop';
  ...
  process.env.ZEDCODE_OPENCODE_CWD = resolveManagedOpenCodeCwd({
    env: process.env,
    homedir: () => os.homedir(),
  });
  process.env.ZEDCODE_DESKTOP_NOTIFY = 'true';
  if (desktopUiPassword) {
    process.env.ZEDCODE_UI_PASSWORD = desktopUiPassword;
  } else {
    delete process.env.ZEDCODE_UI_PASSWORD;
  }
  process.env.ZEDCODE_SKIP_API_COMPRESSION = process.env.ZEDCODE_SKIP_API_COMPRESSION || 'true';
  process.env.NO_PROXY = process.env.NO_PROXY || 'localhost,127.0.0.1';
  process.env.no_proxy = process.env.no_proxy || 'localhost,127.0.0.1';

  const { startWebUiServer } = await import('@zedcode/web/server/index.js');
...
`;
  ...
  const child = spawn('/bin/sh', ['-c', script, 'zedcode-opencode-killer', normalizedPid, normalizedPort, String(OPENCODE_SHUTDOWN_GRACE_MS / 1000)], {
    detached: true,
    stdio: 'ignore',
    windowsHide: true,
  });
...
const buildInitScript = (localOrigin, bootOutcome, apiBaseUrl = '', clientToken = '', requestHeaders = {}) => {
  ...
  return [
    '(function(){',
    `try{var __oc_local=${local};var __oc_api=${apiBase};var __oc_headers=${headers};var __oc_packaged=${packagedOrigin};var __oc_origin=window.location&&window.location.origin||'';var __oc_is_packaged=__oc_origin===__oc_packaged;var __oc_is_local=__oc_local&&__oc_origin===new URL(__oc_local).origin;window.__ZEDCODE_MACOS_MAJOR__=${macVersion};window.__ZEDCODE_LOCAL_ORIGIN__=__oc_local;window.__ZEDCODE_API_BASE_URL__=__oc_api;if(__oc_is_local||__oc_is_packaged){window.__ZEDCODE_HOME__=${home};window.__ZEDCODE_RUNTIME_HEADERS__=__oc_headers;}if((__oc_is_local||__oc_is_packaged)&&${token}){window.__ZEDCODE_CLIENT_TOKEN__=${token};}var __oc_bo=${outcome};if(__oc_bo){window.__ZEDCODE_DESKTOP_BOOT_OUTCOME__=__oc_bo;}}catch(_e){}`,
    '}())',
  ].join('');
};
...
const buildStartupSplashHtml = () => {
  ...
  <body>
    <div class="stack">
      <svg width="120" height="120" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ZedCode loading icon">
...
const emitToWindow = (browserWindow, event, detail) => {
  if (!browserWindow || browserWindow.isDestroyed()) return;
  browserWindow.webContents.send('zedcode:emit', { event, detail });
};
...
const parseConnectPairingDeepLinkPayload = (raw) => {
  ...
  try {
    ...
    return {
      pairingId,
      secret,
      label: typeof payload.label === 'string' && payload.label.trim() ? payload.label.trim() : 'ZedCode',
...
const confirmConnectDeepLink = async (payload) => {
  ...
  const options = {
    type: 'warning',
    title: 'Connect to ZedCode server?',
...
const dispatchDeepLink = (link) => {
  ...
  if (link.type === 'focus') {
    ...
    emitToAllWindows('zedcode:deep-link-focus', { reason: link.value || null });
    return;
  }

  if (link.type === 'session' && link.value) {
    emitToAllWindows('zedcode:open-session', { sessionId: link.value });
    return;
  }
...
const dispatchMenuAction = (action) => {
  ...
  emitToWindow(target, 'zedcode:menu-action', action);
  dispatchDomEventToWindow(target, 'zedcode:menu-action', action);
};
...
const dispatchAddSelectionToChat = () => {
  const target = getMenuTargetWindow();
  if (target) emitToWindow(target, 'zedcode:menu-action', 'add-selection-to-chat');
};
...
const dispatchOpenMiniChat = (browserWindow) => {
  ...
  if (target) emitToWindow(target, 'zedcode:open-mini-chat');
};

const dispatchCheckForUpdates = () => {
  emitToAllWindows('zedcode:check-for-updates');
  for (const browserWindow of BrowserWindow.getAllWindows()) {
    dispatchDomEventToWindow(browserWindow, 'zedcode:check-for-updates');
  }
};
...
const createBrowserWindow = ({ label, restoreGeometry, url, runtimeConfig = {} }) => {
  ...
  const options = {
    ...
    webPreferences: {
      additionalArguments: [
        `--zedcode-local-origin=${desktopLocalOrigin}`,
        `--zedcode-api-base-url=${desktopApiBaseUrl}`,
        `--zedcode-client-token=${desktopClientToken}`,
        `--zedcode-runtime-headers=${JSON.stringify(desktopRequestHeaders)}`,
        `--zedcode-home=${desktopHome}`,
        `--zedcode-macos-major=${desktopMacosMajor}`,
        `--zedcode-tray-enabled=${trayEnabled ? '1' : '0'}`,
        `--zedcode-boot-outcome=${JSON.stringify(state.bootOutcome || null)}`,
        `--zedcode-relay-host-id=${rendererRuntimeConfig.relayHostId || ''}`,
      ],
  ...
  browserWindow.on('resize', () => {
    if (process.platform === 'darwin') {
      emitToWindow(browserWindow, 'zedcode:window-resized');
    }
    debounceWindowStatePersist(browserWindow, false);
  });
  browserWindow.on('maximize', () => {
    emitToWindow(browserWindow, 'zedcode:window-maximized-changed', { maximized: true });
    debounceWindowStatePersist(browserWindow, false);
  });
  browserWindow.on('unmaximize', () => {
    emitToWindow(browserWindow, 'zedcode:window-maximized-changed', { maximized: false });
    debounceWindowStatePersist(browserWindow, false);
  });
...
const resolveInitialUrl = async () => {
  const hmrApiPort = process.env.ZEDCODE_HMR_API_PORT || '3901';
  const hmrUiPort = process.env.ZEDCODE_HMR_UI_PORT || '5173';
  ...
  const envTarget = normalizeHostUrl(process.env.ZEDCODE_SERVER_URL || '');
  const config = readDesktopHostsConfig();
  if (envTarget) {
  ...
  if (!initialUrl) {
    throw new Error(
      'ZEDCODE_SKIP_LOCAL_SERVER=1 requires bundled UI, a running desktop HMR UI, or a reachable remote instance.',
    );
  }
...
const setupAutoUpdater = () => {
  ...
  const testBuild = typeof __ZEDCODE_UPDATER_E2E_BUILD__ !== 'undefined'
    && __ZEDCODE_UPDATER_E2E_BUILD__ === true;
  ...
  autoUpdater.on('download-progress', (progress) => {
    ...
    emitToAllWindows('zedcode:update-progress', mapUpdaterProgressEvent({
      event: 'Progress',
      data: {
...
const iconToDataUrl = async (iconPath, appName) => {
  ...
  const tempPath = path.join(os.tmpdir(), `zedcode-icon-${safeName}-${Date.now()}.png`);
  try {
...
const getDevTunnelClient = async () => {
  if (!devTunnelClientPromise) {
    devTunnelClientPromise = import('@zedcode/web/server/lib/dev-tunnel/client.js')
...
const handleInvoke = async (browserWindow, command, args = {}) => {
  switch (command) {
    ...
    // on the remote ZedCode host, so the browser panel loads a real origin
     ...
     *
     * nativeTheme.themeSource is app-wide and would drag ZedCode's own
     ...
     * to third-party hosts from ZedCode itself. The bytes come back as a
    ...
    // touch ZedCode's session or any other window's storage.
    ...
    case 'desktop_read_file': {
      ...
      const DENIED_SEGMENTS = ['.ssh', '.aws', '.gnupg', '.gpg', '.config/gh', '.config/zedcode/credentials'];
    ...
    case 'desktop_get_installed_apps': {
      ...
      const refresh = async () => {
        ...
        emitToAllWindows('zedcode:installed-apps-updated', apps);
      };
      ...
      if (applyUpdate && process.platform === 'darwin' && typeof app.isInApplicationsFolder === 'function') {
        try {
          if (!app.isInApplicationsFolder()) {
            throw new Error('Desktop update requires ZedCode.app to be installed in /Applications');
          }
        } catch (error) {
    ...
    case 'desktop_focus_main_window': {
      ...
      if (sessionId) {
        emitToWindow(state.mainWindow, 'zedcode:open-session', { sessionId, directory });
      } else if (mode === 'draft') {
        emitToWindow(state.mainWindow, 'zedcode:open-draft-session', { directory, projectId });
      }
      return { focused: true };
    }
  ...
  return Menu.buildFromTemplate([
    {
      label: app.name,
      submenu: [
        { label: 'About ZedCode', click: () => dispatchAction('about') },
        {
...
const isLocalSender = (webContents) => {
  try {
    ...
    if (isDev && url.origin === `http://127.0.0.1:${process.env.ZEDCODE_HMR_UI_PORT || '5173'}`) return true;
    if (url.protocol !== 'http:' && url.protocol !== 'https:') return false;
    if (state.localOrigin) {
      try {
...
ipcMain.handle('zedcode:invoke', async (event, command, args) => {
...
ipcMain.handle('zedcode:dialog:open', async (event, options) => {
...
ipcMain.handle('zedcode:file:grant-existing', async (event, filePath) => {
  if (!isLocalSender(event.sender)) {
...
const focusMainWindowWithSession = async (sessionId, directory) => {
  if (state.mainWindow && !state.mainWindow.isDestroyed()) {
    ...
    state.mainWindow.focus();
    if (sessionId) {
      emitToWindow(state.mainWindow, 'zedcode:open-session', { sessionId, directory: directory || '' });
    }
    return;
  }
  if (sessionId) pendingDeepLinks.push({ type: 'session', value: sessionId });
...
const dispatchTrayAction = async (action) => {
  ...
  if (action.type === 'respond-permission') {
    const target = (state.mainWindow && !state.mainWindow.isDestroyed())
      ? state.mainWindow
      : await revealMainWindow();
    emitToWindow(target, 'zedcode:tray-action', action);
    return;
  }
  ...
  if (action.type === 'focus-session') {
    ...
    if (surface && surface.__ocMiniChat === true && action.sessionId) {
      ...
      surface.show();
      surface.focus();
      emitToWindow(surface, 'zedcode:open-session', {
  ...
  if (action.type === 'new-session') {
    emitToWindow(target, 'zedcode:open-draft-session', { directory: '', projectId: '' });
  }
...
app.whenReady().then(async () => {
  ...
  powerMonitor.on('resume', () => {
    emitToAllWindows('zedcode:system-resume', { timestamp: Date.now() });
  });
}).catch((error) => {
