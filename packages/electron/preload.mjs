...
const localOrigin = readArgValue('--zedcode-local-origin');
const apiBaseUrl = readArgValue('--zedcode-api-base-url');
const clientToken = readArgValue('--zedcode-client-token');
const runtimeHeadersRaw = readArgValue('--zedcode-runtime-headers');
const homeDirectory = readArgValue('--zedcode-home');
const macosMajorRaw = readArgValue('--zedcode-macos-major');
const macosMajor = Number.parseInt(macosMajorRaw, 10);
const trayEnabled = process.platform !== 'darwin' || readArgValue('--zedcode-tray-enabled') !== '0';
...
//  - __ZEDCODE_ELECTRON__ is a shell-identity flag (no capability).
...
//  - __ZEDCODE_DESKTOP__ is the IPC channel to the main process. It is
...
const isLocalPage = currentOrigin !== 'null'
  && (currentOrigin === 'zedcode-ui://app'
  || (localOrigin && currentOrigin === localOrigin));

// Remote pages need __ZEDCODE_LOCAL_ORIGIN__ so the HostSwitcher knows
...
if (localOrigin) {
  contextBridge.exposeInMainWorld('__ZEDCODE_LOCAL_ORIGIN__', localOrigin);
}

if (apiBaseUrl) {
  contextBridge.exposeInMainWorld('__ZEDCODE_API_BASE_URL__', apiBaseUrl);
}

if (clientToken && isLocalPage) {
  contextBridge.exposeInMainWorld('__ZEDCODE_CLIENT_TOKEN__', clientToken);
}
...
const relayHostId = readArgValue('--zedcode-relay-host-id');
if (relayHostId && isLocalPage) {
  contextBridge.exposeInMainWorld('__ZEDCODE_RELAY_HOST_ID__', relayHostId);
}
...
if (runtimeHeadersRaw && isLocalPage) {
  try {
    ...
    if (runtimeHeaders && typeof runtimeHeaders === 'object') {
      contextBridge.exposeInMainWorld('__ZEDCODE_RUNTIME_HEADERS__', runtimeHeaders);
    }
  } catch {
  }
}
...
if (isLocalPage && homeDirectory) {
  contextBridge.exposeInMainWorld('__ZEDCODE_HOME__', homeDirectory);
}
...
if (Number.isFinite(macosMajor) && macosMajor > 0) {
  contextBridge.exposeInMainWorld('__ZEDCODE_MACOS_MAJOR__', macosMajor);
}

contextBridge.exposeInMainWorld('__ZEDCODE_ELECTRON__', {
  runtime: 'electron',
  arch: process.arch,
  trayEnabled,
});

contextBridge.exposeInMainWorld('__ZEDCODE_PLATFORM__', process.platform);
...
ipcRenderer.on('zedcode:emit', (_evt, payload) => {
...
// ipcMain.handle('zedcode:invoke') decides per-command what is safe
// for non-local callers (window/host-switcher ops yes, file/shell ops
// no). See COMMANDS_SAFE_FOR_REMOTE in main.mjs.
contextBridge.exposeInMainWorld('__ZEDCODE_DESKTOP__', {
  invoke: (cmd, args) => ipcRenderer.invoke('zedcode:invoke', cmd, args || {}),
  openDialog: (options) => ipcRenderer.invoke('zedcode:dialog:open', options || {}),
  grantFileAccess: (filePath) => ipcRenderer.invoke('zedcode:file:grant-existing', filePath),
  openExternal: (url) => ipcRenderer.invoke('zedcode:invoke', 'desktop_open_external_url', { url }),
