...
test('prefers APPIMAGE path for Linux autostart Exec', () => {
  assert.equal(
    resolveLinuxLaunchExecutable({
      env: { APPIMAGE: '/home/user/ZedCode.AppImage' },
      execPath: '/tmp/.mount_OpenChXXXX/zedcode',
    }),
    '/home/user/ZedCode.AppImage',
  );
});
...
test('writes and removes the XDG autostart file', async () => {
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'zedcode-autostart-'));
  ...
  try {
    ...
    const enabled = await setLinuxAutostartEnabled({
      enabled: true,
      backgroundArg: '--background',
      env: { ...env, APPIMAGE: '/opt/ZedCode.AppImage' },
      homeDir,
    });
    ...
    assert.match(contents, /Exec=\/opt\/ZedCode\.AppImage --background/);
