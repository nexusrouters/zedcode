...
const fixture = (manifestName, artifactName, fields) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zedcode-manifest-test-'));
...
for (const [manifestName, artifactName] of [
  ['latest-linux.yml', 'ZedCode-1.15.0-linux-x86_64.AppImage'],
  ['latest-linux-arm64.yml', 'ZedCode-1.15.0-linux-arm64.AppImage'],
]) {
...
test('accepts electron-builder field ordering and optional blockMapSize', () => {
  const artifactName = 'ZedCode-1.15.0-linux-x86_64.AppImage';
  const bytes = Buffer.from(`artifact:${artifactName}`);
...
test('rejects a manifest that points at the other architecture artifact', () => {
  const value = fixture('latest-linux-arm64.yml', 'ZedCode-1.15.0-linux-arm64.AppImage');
  try {
    const x64Artifact = path.join(value.root, 'ZedCode-1.15.0-linux-x86_64.AppImage');
