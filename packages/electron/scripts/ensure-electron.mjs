 ...
 *   ZEDCODE_ELECTRON_PKG_DIR       - resolve the electron package here.
 *   ZEDCODE_ELECTRON_INSTALL_COMMANDS - JSON array of [bin, args] repair
 *                                        commands, e.g.
...
function resolveInstallCommands(env) {
  if (env.ZEDCODE_ELECTRON_INSTALL_COMMANDS) {
    try {
      const parsed = JSON.parse(env.ZEDCODE_ELECTRON_INSTALL_COMMANDS);
      if (
...
export async function main(argv = process.argv.slice(2), env = process.env) {
  ...
  const overrideDir = env.ZEDCODE_ELECTRON_PKG_DIR;
