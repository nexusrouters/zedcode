...
//
// Surfaces a glanceable, always-visible view of ZedCode's live state:
...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'ZedCode — no active sessions';
  const bits = [];
  ...
  return `ZedCode — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    if (isLinux || process.platform === 'win32') {
      ...
      template.push(
        { label: 'Show ZedCode', click: () => onAction({ type: 'show-main-window' }) },
        { type: 'separator' },
        { label: 'Quit ZedCode', click: () => onAction({ type: 'quit' }) },
      );
    }
