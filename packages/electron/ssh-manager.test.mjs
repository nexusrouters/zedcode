...
describe('ElectronSshManager', () => {
  ...
  test('creates a PowerShell-backed askpass helper on Windows', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zedcode-ssh-askpass-test-'));
    ...
    expect(await fsp.readFile(path.join(tempDir, 'askpass.ps1'), 'utf8')).toContain('ZEDCODE_SSH_ASKPASS_VALUE');
  });
  ...
  test('runs each Windows port forward as an independent hidden SSH process', async () => {
    ...
    manager.sshAuth.set(parsed, {
      askpassPath: 'C:\\ZedCode\\askpass.cmd',
    ...
    for (const call of calls) {
      ...
      expect(call.options.env.SSH_ASKPASS).toBe('C:\\ZedCode\\askpass.cmd');
      expect(call.options.env.ZEDCODE_SSH_ASKPASS_VALUE).toBe('secret-value');
    }
  ...
  test('stops in-flight commands and forwards when disconnecting Windows SSH', async () => {
    ...
    manager.sessions.set('ssh-1', {
      instance: { remoteZedcode: { mode: 'external', keepRunning: true } },
      parsed,
  ...
  test('stores a client token for forwarded ZedCode hosts when UI password is configured', async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zedcode-ssh-manager-test-'));
    tempDirs.push(tempDir);
