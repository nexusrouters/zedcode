...
const CHAT_FORCE_SCROLL_BOTTOM_EVENT = 'zedcode:chat-force-scroll-bottom';
...
export const ChatContainer: React.FC<ChatContainerProps> = ({
    ...
    React.useEffect(() => {
        ...
        const scopedWindow = window as typeof window & {
            __zedcodeApplyChatSettingsSync?: (payload: { allowPromptingSubagentSessions: boolean }) => void;
        };
        ...
        const handleMessage = (event: MessageEvent) => {
            ...
            if (data?.type !== 'zedcode:chat-settings-sync'
        ...
        scopedWindow.__zedcodeApplyChatSettingsSync = applySync;
        window.addEventListener('message', handleMessage);
        parentWindow.postMessage({ type: 'zedcode:chat-settings-request' }, window.location.origin);
        return () => {
            window.removeEventListener('message', handleMessage);
            if (scopedWindow.__zedcodeApplyChatSettingsSync === applySync) {
                delete scopedWindow.__zedcodeApplyChatSettingsSync;
            }
        };
    }, []);
