...
export const VSCodeLayout: React.FC = () => {
  ...
  const [connectionStatus, setConnectionStatus] = React.useState<'connecting' | 'connected' | 'error' | 'disconnected'>(
    () => (typeof window !== 'undefined'
      ? (window as { __ZEDCODE_CONNECTION__?: { status?: string } }).__ZEDCODE_CONNECTION__?.status as
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('zedcode.setActiveSession', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('zedcode.updateSessionEditorTitle', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    const current =
      (typeof window !== 'undefined'
        ? (window as { __ZEDCODE_CONNECTION__?: { status?: string } }).__ZEDCODE_CONNECTION__?.status
    ...
    window.addEventListener('zedcode:connection-status', handler as EventListener);
    return () => window.removeEventListener('zedcode:connection-status', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('zedcode:navigate', handler as EventListener);
    return () => window.removeEventListener('zedcode:navigate', handler as EventListener);
  }, []);
    ...
    const runBootstrap = async () => {
      ...
      try {
        const debugEnabled = (() => {
          ...
          try {
            return window.localStorage.getItem('zedcode_stream_debug') === '1';
          } catch {
            return false;
          }
        })();

        if (debugEnabled) console.log('[ZedCode][VSCode][bootstrap] attempt', { configInitialized });
        ...
        if (debugEnabled) console.log('[ZedCode][VSCode][bootstrap] post-load', {
