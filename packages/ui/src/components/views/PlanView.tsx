...
import { parseProjectPlanMarkdown } from '@/lib/zedcodeConfig';
...
export const PlanView: React.FC<PlanViewProps> = ({ targetPath = null, onNavigatedToChat }) => {
  ...
  const MD_VIEWER_MODE_KEY = 'zedcode:plan:md-viewer-mode';
  ...
  const handleConfirmPlanSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      try {
        ...
        if (pendingPlanSend.target === 'worktree') {
          ...
          initializeNewZedCodeSession(sessionResult.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, content, createSession, currentProjectRef, initializeNewZedCodeSession, pendingPlanSend, resolvedPath, routeToChat, sendMessage, sendPromptTitle, setCurrentSession]
  );
