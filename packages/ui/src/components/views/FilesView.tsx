...
export const FilesView: React.FC<FilesViewProps> = ({ mode = 'full' }) => {
  ...
  const MD_VIEWER_MODE_KEY = 'zedcode:files:md-viewer-mode';
  const HTML_VIEWER_MODE_KEY = 'zedcode:files:html-viewer-mode';
  const JSON_VIEWER_MODE_KEY = 'zedcode:files:json-viewer-mode';
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('zedcode:file-viewer-preview-mode-changed', handleFileViewerModeChanged);
    return () => {
      window.removeEventListener('zedcode:file-viewer-preview-mode-changed', handleFileViewerModeChanged);
    };
  }, [openPaths]);
