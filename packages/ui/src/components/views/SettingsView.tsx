...
import type { ZedCodeSection } from '@/components/sections/zedcode/types';
import { ZedCodePage } from '@/components/sections/zedcode/ZedCodePage';
import { AboutSettings } from '@/components/sections/zedcode/AboutSettings';
...
const SETTINGS_DETAIL_HISTORY_KEY = '__zedcodeSettingsDetail';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ onClose, forceMobile, isWindowed, visiblePageSlugs, initialMobileStage = 'nav' }) => {
  ...
  const isWindows = React.useMemo(() => {
    return isDesktopShell() && typeof window !== 'undefined'
      && (window as unknown as { __ZEDCODE_PLATFORM__?: string }).__ZEDCODE_PLATFORM__ === 'win32';
  }, []);
  const isLinux = React.useMemo(() => {
    return isDesktopShell() && typeof window !== 'undefined'
      && (window as unknown as { __ZEDCODE_PLATFORM__?: string }).__ZEDCODE_PLATFORM__ === 'linux';
  }, []);
  ...
  const zedCodeSectionBySlug: Partial<Record<SettingsPageSlug, ZedCodeSection>> = React.useMemo(() => ({
    general: 'general',
    appearance: 'visual',
    chat: 'chat',
    shortcuts: 'shortcuts',
  ...
  const openSearchResult = React.useCallback((result: SettingsSearchResult) => {
    ...
    if (result.id === 'plugins.create' && typeof window !== 'undefined') {
      window.setTimeout(() => {
        window.dispatchEvent(new CustomEvent('zedcode:settings-open-plugin-add'));
      }, 50);
    }
  ...
  const renderPageContent = React.useCallback((slug: SettingsPageSlug) => {
    ...
    switch (slug) {
      ...
      case 'voice':
      case 'tunnel': {
        const section = zedCodeSectionBySlug[slug] ?? 'visual';
        return <ZedCodePage section={section} />;
      }
      case 'home':
      default:
        return null;
    }
  }, [zedCodeSectionBySlug, openPage, openThirdPartyProviderSetup, renderUnavailable, runtimeCtx, t]);
