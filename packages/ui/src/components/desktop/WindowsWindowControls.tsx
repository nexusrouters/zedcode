...
// foreign platform's chrome, not ZedCode's own status tokens, so the
...
export const WindowsWindowControls = React.memo(function WindowsWindowControls({
  ...
  useEffect(() => {
    ...
    window.addEventListener('zedcode:window-maximized-changed', handleMaximizedChange);
    return () => {
      disposed = true;
      window.removeEventListener('zedcode:window-maximized-changed', handleMaximizedChange);
    };
  }, [visible]);
