...
import { ZedCodeVisualSettings } from './ZedCodeVisualSettings';
...
import { ZedCodeToolsSettings } from './ZedCodeToolsSettings';
...
import type { ZedCodeSection } from './types';
...
interface ZedCodePageProps {
    /** Which section to display. If undefined, shows all sections (mobile/legacy behavior) */
    section?: ZedCodeSection;
}

export const ZedCodePage: React.FC<ZedCodePageProps> = ({ section }) => {
    const { t } = useI18n();
    ...
    if (!section) {
        return (
            <SettingsPageLayout showSaveStatus className="zedcode-page-body space-y-3 sm:space-y-6">
                <ZedCodeVisualSettings />
                <DefaultsSettings />
                {showDesktopNetworkSettings && <DesktopNetworkSettings />}
                {!isVSCode && <OpenCodeCliSettings />}
                {!isVSCode && <ZedCodeToolsSettings />}
...
const GeneralSectionContent: React.FC = () => {
    ...
    return (
        <>
            ...
            {!isVSCode && <OpenCodeCliSettings />}
            {!isVSCode && <ZedCodeToolsSettings />}
            <ZedCodeVisualSettings visibleSettings={[
                'fileEditorKeymap',
                'autoSaveEnabled',
...
const VisualSectionContent: React.FC = () => {
    const isVSCode = isVSCodeRuntime();
    return <ZedCodeVisualSettings visibleSettings={[
        'theme',
