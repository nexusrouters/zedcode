...
import { ZedCodeLogo } from '@/components/ui/ZedCodeLogo';
...
const GITHUB_URL = 'https://github.com/zedcode/zedcode';
const DISCORD_URL = 'https://discord.gg/ZYRSdnwwKA';
const X_URL = 'https://x.com/zedcode_dev';
...
export const AboutSettings: React.FC<AboutSettingsProps> = ({ initialUpdateDialogOpen = false }) => {
  ...
  const [zedCodeVersion, setZedCodeVersion] = React.useState<string | null>(null);
  ...
  const currentVersion = zedCodeVersion || updateStore.info?.currentVersion || 'unknown';

  React.useEffect(() => {
    let cancelled = false;

    const loadZedCodeVersion = async () => {
      try {
        ...
        const data = await response.json().catch(() => null) as { zedcodeVersion?: unknown } | null;
        const version = typeof data?.zedcodeVersion === 'string' && data.zedcodeVersion.trim().length > 0
          ? data.zedcodeVersion.trim()
          : null;
        if (!cancelled) setZedCodeVersion(version);
      } catch {
        if (!cancelled) setZedCodeVersion(null);
      }
    };

    void loadZedCodeVersion();
    ...
    if (updateStore.checking) {
      ...
      const timer = setTimeout(() => {
        ...
        if (didInitiateCheck.current && !updateStore.available && !updateStore.error) {
          toast.success(t('settings.zedcode.about.toast.latestVersion'));
    ...
    return (
      <div className="w-full space-y-6 pb-2">
        <div className="flex flex-col items-center text-center">
          <ZedCodeLogo width={72} height={72} />
          <h2 className={`mt-4 ${SETTINGS_BRAND_TITLE_CLASS}`}>ZedCode</h2>
          <div className="mt-2 space-y-1 typography-ui text-muted-foreground">
            <p>{t('aboutDialog.zedCodeVersionLabel', { version: currentVersion })}</p>
            <p>{t('aboutDialog.openCodeVersionLabel', { version: openCodeVersion || t('settings.zedcode.about.state.unknown') })}</p>
          </div>
          <InstanceServiceUrls />
        </div>
        ...
        <div className="flex justify-center">
          {!updateStore.available && !updateStore.error && (
            ...
            >
              ...
              {isChecking ? t('settings.zedcode.about.state.checking') : t('settings.zedcode.about.actions.checkForUpdates')}
            </Button>
          )}
          ...
          {!isChecking && updateStore.available && (
            ...
            >
              ...
              {t('settings.zedcode.about.actions.updateToVersion', { version: updateStore.info?.version || '' })}
            </Button>
          )}
        </div>
      ...
      <div className="rounded-lg bg-[var(--surface-elevated)]/70 overflow-hidden flex flex-col">
        <div className="flex flex-col @xl:flex-row @xl:items-center justify-between gap-4 px-4 py-3 border-b border-border/40">
          <div className="flex min-w-0 flex-col">
            <span className={SETTINGS_FIELD_LABEL_CLASS}>{t('settings.zedcode.about.field.version')}</span>
            <span className="typography-meta text-muted-foreground font-mono">{currentVersion}</span>
          </div>
          <div className="flex min-w-0 flex-col">
            <span className={SETTINGS_FIELD_LABEL_CLASS}>{t('settings.zedcode.about.field.openCodeVersion')}</span>
            <span className="typography-meta text-muted-foreground font-mono">{openCodeVersion || t('settings.zedcode.about.state.unknown')}</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            {updateStore.checking && (
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="typography-meta">{t('settings.zedcode.about.state.checking')}</span>
              </div>
            )}
            ...
            {!updateStore.checking && !updateStore.available && !updateStore.error && (
              <span className="typography-meta text-muted-foreground">{t('settings.zedcode.about.state.upToDate')}</span>
            )}
            ...
            >
              {t('settings.zedcode.about.actions.checkForUpdates')}
            </Button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 border-b border-border/40 px-4 py-3 @xl:flex-row @xl:items-center @xl:justify-between">
          <span className={SETTINGS_FIELD_LABEL_CLASS}>{t('settings.zedcode.about.field.instanceUrls')}</span>
          <InstanceServiceUrls />
        </div>
