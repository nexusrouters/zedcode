...
export const OpenCodeCliSettings: React.FC = () => {
  ...
  return (
    <SettingsSection title={t('settings.zedcode.opencodeCli.title')}>
      <div className="space-y-0.5">
        <SettingsFieldRow
          settingsItem="sessions.opencode-binary"
          label={t('settings.zedcode.opencodeCli.field.binaryPath')}
          info={(
            <>
              {t('settings.zedcode.opencodeCli.tipPrefix')}
              {' '}
              <span className="font-mono">OPENCODE_BINARY</span>
              {' '}
              {t('settings.zedcode.opencodeCli.tipMiddle')}
              {' '}
              <span className="font-mono">~/.config/zedcode/settings.json</span>
              {'.'}
            </>
          )}
          alignEnd={false}
        ...
        >
          <Input
            ...
            placeholder={t('settings.zedcode.opencodeCli.field.binaryPathPlaceholder')}
          ...
          <Button
            ...
            aria-label={t('settings.zedcode.opencodeCli.actions.browseAria')}
            title={t('settings.zedcode.opencodeCli.actions.browse')}
          >
        ...
        <SettingsInset className={SETTINGS_OPTION_STACK_CLASS}>
          {!isWindowsArm64() && (
            <SettingsCheckboxRow
              ...
              label={t('settings.zedcode.opencodeCli.field.showUpdateNotifications')}
              ariaLabel={t('settings.zedcode.opencodeCli.field.showUpdateNotificationsAria')}
            />
          )}
