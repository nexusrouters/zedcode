    ...
    const fetchVersion = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (typeof data.zedcodeVersion === 'string' && data.zedcodeVersion.trim()) {
            setVersion(data.zedcodeVersion);
            return;
          }
        }
      } catch {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xs p-6">
        <div className="flex flex-col items-center text-center space-y-4">
          <ZedCodeLogo width={64} height={64} />

          <div className="space-y-1">
            <h2 className="text-lg font-semibold">ZedCode</h2>
            <div className="space-y-0.5 typography-meta text-muted-foreground">
              {displayVersion && (
                <p>{t('aboutDialog.zedCodeVersionLabel', { version: displayVersion })}</p>
              )}
              {openCodeVersion && (
          ...
          <div className="flex flex-col items-center gap-2 pt-2">
            <div className="flex items-center justify-center gap-4">
              <a
                href="https://github.com/zedcode/zedcode"
                target="_blank"
