...
const TRAY_ACTION_EVENT = 'zedcode:tray-action';
...
type TraySnapshot = {
  ...
  // Active instance label (e.g. "Local ZedCode" or a remote host name) so
...
// the existing `zedcode:open-session` / `zedcode:open-draft-session`
...
const isTrayPlatform = (): boolean => {
  ...
  const platform = (window as unknown as { __ZEDCODE_PLATFORM__?: string }).__ZEDCODE_PLATFORM__;
  return platform === 'darwin' || platform === 'win32' || platform === 'linux';
};

const isTrayEnabled = (): boolean =>
  typeof window !== 'undefined' && window.__ZEDCODE_ELECTRON__?.trayEnabled !== false;
...
// the local origin shows as "Local ZedCode"; a remote host shows its
// configured name. Async because the host config is read over IPC.
const resolveInstanceName = async (): Promise<string> => {
  try {
    if (isDesktopLocalOriginActive()) return 'Local ZedCode';
    const localOrigin = (window as unknown as { __ZEDCODE_LOCAL_ORIGIN__?: string }).__ZEDCODE_LOCAL_ORIGIN__
      || window.location.origin;
    const runtimeApiBaseUrl = getRuntimeApiBaseUrl();
    if (runtimeApiBaseUrl && locationMatchesHost(runtimeApiBaseUrl, localOrigin)) return 'Local ZedCode';
...
export const useTraySync = (): void => {
  ...
  React.useEffect(() => {
    ...
    const bridge = (window as unknown as { __ZEDCODE_DESKTOP__?: DesktopBridgeGlobal }).__ZEDCODE_DESKTOP__;
