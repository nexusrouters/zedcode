...
const MODELS_DEV_PROXY_URL = "/api/zedcode/models-metadata";
...
const fetchZedCodeDefaults = async (): Promise<ZedCodeDefaults> => {
    markStartupTrace('config.defaults:start');
    const started = typeof performance !== 'undefined' ? performance.now() : Date.now();
    const finish = (source: string, result: ZedCodeDefaults) => {
...
interface ConfigStore {
    ...
    // ZedCode settings-based defaults (take precedence over agent preferences)
        ...
        persist(
            (set, get) => ({
                ...
                loadAgents: async (options) => {
                    ...
                    for (let attempt = 0; attempt < 3; attempt++) {
                        try {
                            // Fetch agents and ZedCode settings in parallel. OpenCode config
                            ...
                            const [agents, zedCodeDefaults] = await Promise.all([
                                measureStartupTrace(
                                    'loadAgents:api',
                                    () => opencodeClient.listAgents(configDirectoryPath),
                                    { directoryKey, source, requestedDirectory, effectiveDirectory, attempt: attempt + 1 },
                                ),
                                fetchZedCodeDefaults(),
                            ]);
                            ...
                            const defaultZenModel = normalizeOptionalString(zedCodeDefaults.zenModel);
                            ...
                            set((state) => {
                                ...
                                const nextState: Partial<ConfigStore> = {
                                    settingsDefaultModel: zedCodeDefaults.defaultModel,
                                    settingsDefaultVariant: zedCodeDefaults.defaultVariant,
                                    settingsDefaultAgent: zedCodeDefaults.defaultAgent,
                                    settingsAutoCreateWorktree: zedCodeDefaults.autoCreateWorktree ?? false,
                                    settingsGitmojiEnabled: zedCodeDefaults.gitmojiEnabled ?? false,
                                    settingsDefaultFileViewerPreview: zedCodeDefaults.defaultFileViewerPreview ?? false,
                                    settingsZenModel: resolvedZenModel,
                                    settingsMessageStreamTransport: zedCodeDefaults.messageStreamTransport ?? state.settingsMessageStreamTransport ?? 'auto',
                                    sttProvider: zedCodeDefaults.sttProvider ?? state.sttProvider,
                                    sttServerUrl: zedCodeDefaults.sttServerUrl ?? state.sttServerUrl,
                                    sttModel: zedCodeDefaults.sttModel ?? state.sttModel,
                                    sttLocalModel: zedCodeDefaults.sttLocalModel ?? state.sttLocalModel,
                                    sttLanguage: zedCodeDefaults.sttLanguage ?? state.sttLanguage,
                                    directoryScoped: {
                                        ...state.directoryScoped,
                                        [directoryKey]: nextSnapshot,
                                    },
                            ...
                            // Detect invalid ZedCode settings so we can clear them from storage.
                            ...
                            if (zedCodeDefaults.defaultAgent && !safeAgents.some((agent) => agent.name === zedCodeDefaults.defaultAgent)) {
                                invalidSettings.defaultAgent = '';
                            }
                            if (zedCodeDefaults.defaultModel) {
                                const parsed = parseModelString(zedCodeDefaults.defaultModel);
                                if (!parsed || !validateModel(parsed.providerId, parsed.modelId)) {
                                    invalidSettings.defaultModel = '';
                                } else if (zedCodeDefaults.defaultVariant) {
                                    const provider = providers.find((p) => p.id === parsed.providerId);
                                    const model = provider?.models.find((m) => m.id === parsed.modelId) as { variants?: Record<string, unknown> } | undefined;
                                    const variants = model?.variants;
                                    if (!(variants && Object.prototype.hasOwnProperty.call(variants, zedCodeDefaults.defaultVariant))) {
                ...
                setAgent: (agentName: string | undefined) => {
                    ...
                    if (agentName) {
                        ...
                        if (currentSessionId && useSessionUIStore.getState().isZedCodeCreatedSession(currentSessionId)) {
                            const existingAgentModel = selState.getAgentModelForSession(currentSessionId, agentName);
                            if (!existingAgentModel) {
                                useSessionUIStore.getState().initializeNewZedCodeSession(currentSessionId, agents);
                            }
                        }
                    }
