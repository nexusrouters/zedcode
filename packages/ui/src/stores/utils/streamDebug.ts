...
const STREAM_PERF_STORAGE_KEY = 'zedcode_stream_perf';
...
declare global {
    interface Window {
        __zedcodeStreamPerfState?: StreamPerfState;
        __zedcodeStreamPerformance?: {
            setEnabled: (enabled: boolean) => void;
            reset: () => void;
            getSnapshot: () => StreamPerfSnapshot;
        };
        __zedcodeVsCodeStreamPerfState?: {
            counters: Map<string, PerfCounter>;
            lastReportAt?: number;
            lastUpdatedAt?: number;
...
const ensureStreamPerfState = (): StreamPerfState | null => {
    ...
    if (!window.__zedcodeStreamPerfState) {
        const startedAt = Date.now();
        window.__zedcodeStreamPerfState = {
    ...
    return window.__zedcodeStreamPerfState;
};
...
export const setStreamPerfEnabled = (enabled: boolean): void => {
    ...
    try {
        ...
        delete window.__zedcodeStreamPerfState;
        delete window.__zedcodeVsCodeStreamPerfState;
    } catch {
...
export const resetStreamPerf = (): void => {
    ...
    if (window.__zedcodeVsCodeStreamPerfState) {
        window.__zedcodeVsCodeStreamPerfState = {
            ...window.__zedcodeVsCodeStreamPerfState,
            counters: new Map<string, PerfCounter>(),
            startedAt: Date.now(),
            lastUpdatedAt: Date.now(),
...
export const getStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__zedcodeStreamPerfState;
...
export const getVsCodeStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__zedcodeVsCodeStreamPerfState;
    if (!streamPerfEnabled || !state) {
        return {
            enabled: false,
            startedAt: null,
...
export const streamPerfMark = (metric: string): void => {
    ...
    performance.mark(`zedcode.${metric}`);
};
...
if (typeof window !== 'undefined') {
    window.__zedcodeStreamPerformance = {
