...
const getLocalRuntimeOrigin = (): string => {
  ...
  const value = (window as typeof window & { __ZEDCODE_LOCAL_ORIGIN__?: string }).__ZEDCODE_LOCAL_ORIGIN__;
...
const createVSCodeWorkspaceProjects = (
  ...
  if (streamDebugEnabled()) {
    console.log('[ZedCode][VSCode][projects] Using workspace projects', projects);
  }
...
if (typeof window !== 'undefined') {
  window.addEventListener('zedcode:settings-synced', (event: Event) => {
