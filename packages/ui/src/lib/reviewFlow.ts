...
const requestChatForceScrollBottom = (sessionId: string): void => {
  ...
  window.dispatchEvent(new CustomEvent('zedcode:chat-force-scroll-bottom', {
    detail: { sessionId },
  }));
};
...
const createOrReuseReviewSession = async (originalSessionID: string, directory: string, expectedRuntimeKey?: string): Promise<Session> => {
  ...
  if (existingReviewID) {
    ...
    await patchSessionMetadata(originalSessionID, directory, (metadata) => {
      const next = { ...metadata };
      const zedcode = next.zedcode;
      if (zedcode && typeof zedcode === 'object' && !Array.isArray(zedcode)) {
        const rest = { ...(zedcode as Record<string, unknown>) };
