...
export const applyPersistedHomeDirectoryToWindow = (homeDirectory: string): void => {
  ...
  if (typeof window.__ZEDCODE_HOME__ === 'string' && window.__ZEDCODE_HOME__.length > 0) {
    return;
  }

  try {
    window.__ZEDCODE_HOME__ = homeDirectory;
  } catch {
    /* read-only contextBridge property — leave preload-seeded value */
  }
};

const SETTINGS_MIRROR_INDEX_KEY = 'zedcode.settingsMirror.v2.index';
const SETTINGS_MIRROR_KEY_PREFIX = 'zedcode.settingsMirror.v2:';
...
const persistToLocalStorage = (settings: DesktopSettings) => {
  ...
  if (typeof settings.pwaAppName === 'string') {
    ...
    if (normalized.length > 0) {
      localStorage.setItem('zedcode.pwaName', normalized);
    } else {
      localStorage.removeItem('zedcode.pwaName');
    }
  } else {
    localStorage.removeItem('zedcode.pwaName');
  }
...
const dispatchSettingsSynced = (settings: DesktopSettings): void => {
  ...
  window.dispatchEvent(new CustomEvent<DesktopSettings>('zedcode:settings-synced', { detail: settings }));
};
...
const dispatchSettingsSaveState = (state: 'saving' | 'saved' | 'error'): void => {
  ...
  window.dispatchEvent(new CustomEvent<'saving' | 'saved' | 'error'>('zedcode:settings-save-state', { detail: state }));
};
...
export const syncDesktopSettings = async (): Promise<void> => {
  ...
  const applySettings = async (settings: DesktopSettings) => {
    ...
    // `zedcode:files:auto-save-enabled`. Prefer the hydrated store value and
