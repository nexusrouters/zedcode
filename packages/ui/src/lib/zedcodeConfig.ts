/**
 * ZedCode project-level configuration service.
 * Stores per-project settings in ~/.config/zedcode/<projectId>.json.
 * Migrates from legacy <project>/.zedcode/zedcode.json.
 */
...
const CONFIG_FILENAME = 'zedcode.json';
// LEGACY_PROJECT_CONFIG: legacy per-project config root inside repo.
const LEGACY_CONFIG_DIR = '.zedcode';
const USER_PROJECTS_DIR_SEGMENTS = ['.config', 'zedcode', 'projects'];
...
interface ZedCodeConfig {
  projectPath?: string;
  'setup-worktree'?: string[];
  'setup-worktree-wait'?: boolean;
  projectNotes?: string;
  projectTodos?: ZedCodeProjectTodoItem[];
  projectPlanFiles?: ZedCodeProjectPlanFileLink[];
  projectActions?: ZedCodeProjectAction[];
  projectActionsPrimaryId?: string;
  draftStarters?: DraftStarterRef[];
}

type ZedCodeProjectActionPlatform = 'macos' | 'linux' | 'windows';

export interface ZedCodeProjectAction {
  id: string;
  name: string;
  command: string;
  icon?: string | null;
  platforms?: ZedCodeProjectActionPlatform[];
  autoOpenUrl?: boolean;
  openUrl?: string;
  desktopOpenSshForward?: string;
}

export interface ZedCodeProjectActionsState {
  actions: ZedCodeProjectAction[];
  primaryActionId: string | null;
}

export interface ZedCodeProjectTodoItem {
  id: string;
  text: string;
  completed: boolean;
  createdAt: number;
...
export interface ZedCodeProjectPlanFileLink {
  id: string;
  path: string;
  createdAt: number;
}

export interface ZedCodeProjectPlanFile {
  title: string;
  body: string;
  raw: string;
  path: string;
...
export interface ZedCodeProjectNotesTodos {
  notes: string;
  todos: ZedCodeProjectTodoItem[];
}

export interface ZedCodeProjectContextData extends ZedCodeProjectNotesTodos {
  plans: ZedCodeProjectPlanFileLink[];
}

export const ZEDCODE_PROJECT_NOTES_MAX_LENGTH = 3000;
export const ZEDCODE_PROJECT_TODO_TEXT_MAX_LENGTH = 120;
const ZEDCODE_PROJECT_ACTION_NAME_MAX_LENGTH = 80;
const ZEDCODE_PROJECT_ACTION_COMMAND_MAX_LENGTH = 4000;
const ZEDCODE_PROJECT_ACTION_OPEN_URL_MAX_LENGTH = 2000;
const ZEDCODE_PROJECT_ACTION_DESKTOP_FORWARD_MAX_LENGTH = 300;
const ZEDCODE_PROJECT_PLAN_TITLE_MAX_LENGTH = 160;

const ZEDCODE_ACTION_PLATFORM_SET = new Set<ZedCodeProjectActionPlatform>(['macos', 'linux', 'windows']);
...
const resolveHomeDirectory = async (): Promise<string | null> => {
  ...
  // In some runtimes, window.__ZEDCODE_HOME__ can be workspace/project-root
...
const sanitizeProjectNotes = (value: unknown): string => {
  ...
  return trimToMaxLength(value, ZEDCODE_PROJECT_NOTES_MAX_LENGTH);
};

const sanitizeProjectTodoItems = (value: unknown): ZedCodeProjectTodoItem[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: ZedCodeProjectTodoItem[] = [];
  for (const entry of value) {
    ...
    const text = trimToMaxLength(textRaw.trim(), ZEDCODE_PROJECT_TODO_TEXT_MAX_LENGTH);
    if (!id || !text) {
      continue;
    }
...
const sanitizeProjectPlanFileLinks = (value: unknown): ZedCodeProjectPlanFileLink[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: ZedCodeProjectPlanFileLink[] = [];
...
const sanitizeProjectActionPlatforms = (value: unknown): ZedCodeProjectActionPlatform[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const unique: ZedCodeProjectActionPlatform[] = [];
  const seen = new Set<ZedCodeProjectActionPlatform>();
  for (const entry of value) {
    if (typeof entry !== 'string') {
      continue;
    }
    const normalized = entry.trim().toLowerCase() as ZedCodeProjectActionPlatform;
    if (!ZEDCODE_ACTION_PLATFORM_SET.has(normalized) || seen.has(normalized)) {
      continue;
    }
    seen.add(normalized);
...
const sanitizeProjectActions = (value: unknown): ZedCodeProjectAction[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: ZedCodeProjectAction[] = [];
  const seenIds = new Set<string>();
  ...
  for (const entry of value) {
    ...
    const id = typeof record.id === 'string' ? record.id.trim() : '';
    const name = trimToMaxLength(typeof record.name === 'string' ? record.name.trim() : '', ZEDCODE_PROJECT_ACTION_NAME_MAX_LENGTH);
    const command = trimToMaxLength(typeof record.command === 'string' ? record.command.trim() : '', ZEDCODE_PROJECT_ACTION_COMMAND_MAX_LENGTH);
    ...
    const openUrl = trimToMaxLength(openUrlRaw, ZEDCODE_PROJECT_ACTION_OPEN_URL_MAX_LENGTH);
    ...
    const desktopOpenSshForward = trimToMaxLength(
      ...
      ZEDCODE_PROJECT_ACTION_DESKTOP_FORWARD_MAX_LENGTH
    );
...
} | null | undefined): ZedCodeProjectActionsState => {
...
const sanitizeProjectNotesAndTodos = (value: {
  notes?: unknown;
  todos?: unknown;
} | null | undefined): ZedCodeProjectNotesTodos => {
  return {
...
const sanitizeProjectContextData = (value: {
  ...
  todos?: unknown;
  plans?: unknown;
} | null | undefined): ZedCodeProjectContextData => {
...
const sanitizePlanTitle = (value: string): string => {
  return trimToMaxLength(value.trim(), ZEDCODE_PROJECT_PLAN_TITLE_MAX_LENGTH);
};
 ...
 */
async function readZedCodeConfig(project: ProjectRef): Promise<ZedCodeConfig | null> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return null;
  }
  ...
  const parseConfig = (text: string | null): ZedCodeConfig | null => {
    ...
    try {
      ...
      return parsed as ZedCodeConfig;
    } catch {
      return null;
    }
  };
  ...
  // 2) Migrate legacy <project>/.zedcode/zedcode.json.
  // LEGACY_PROJECT_CONFIG: migrate project-local zedcode.json -> ~/.config/zedcode/projects/<projectId>.json
  ...
  try {
    const wrote = await writeZedCodeConfig(project, legacyConfig);
    if (wrote) {
      await deleteLegacyZedCodeConfig(projectDirectory);
    }
  } catch {
 ...
 */
async function writeZedCodeConfig(
  project: ProjectRef,
  config: ZedCodeConfig
): Promise<boolean> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return false;
  ...
  try {
    ...
    console.error('Failed to write zedcode config:', error);
    return false;
  }
}
 ...
 */
async function updateZedCodeConfig(
  project: ProjectRef,
  updates: Partial<ZedCodeConfig>
): Promise<boolean> {
  const existing = await readZedCodeConfig(project) || {};
  const merged = { ...existing, ...updates };
  return writeZedCodeConfig(project, merged);
}
...
export async function getWorktreeSetupCommands(project: ProjectRef): Promise<string[]> {
  const config = await readZedCodeConfig(project);
...
export async function saveWorktreeSetupCommands(project: ProjectRef, commands: string[]): Promise<boolean> {
  ...
  return updateZedCodeConfig(project, { 'setup-worktree': filtered });
}
...
export async function saveWorktreeSetupWaitEnabled(project: ProjectRef, enabled: boolean): Promise<boolean> {
  return updateZedCodeConfig(project, { 'setup-worktree-wait': enabled });
}
...
export async function saveProjectDraftStarters(project: ProjectRef, starters: DraftStarterRef[]): Promise<boolean> {
  return updateZedCodeConfig(project, { draftStarters: sanitizeStarterRefs(starters) });
}

export async function getProjectNotesAndTodos(project: ProjectRef): Promise<ZedCodeProjectNotesTodos> {
  const config = await readZedCodeConfig(project);
...
export async function saveProjectNotesAndTodos(
  project: ProjectRef,
  value: ZedCodeProjectNotesTodos
): Promise<boolean> {
  ...
  return updateZedCodeConfig(project, {
    projectNotes: sanitized.notes,
    projectTodos: sanitized.todos,
  });
}

export async function getProjectContextData(project: ProjectRef): Promise<ZedCodeProjectContextData> {
  const config = await readZedCodeConfig(project);
...
async function getProjectPlanFiles(project: ProjectRef): Promise<ZedCodeProjectPlanFileLink[]> {
  const config = await readZedCodeConfig(project);
...
async function saveProjectPlanFiles(
  project: ProjectRef,
  value: ZedCodeProjectPlanFileLink[]
): Promise<boolean> {
...
export async function readProjectPlanFile(path: string): Promise<ZedCodeProjectPlanFile | null> {
...
): Promise<ZedCodeProjectPlanFileLink | null> {
...
export async function getProjectActionsState(project: ProjectRef): Promise<ZedCodeProjectActionsState> {
  const config = await readZedCodeConfig(project);
...
export async function saveProjectActionsState(
  project: ProjectRef,
  value: ZedCodeProjectActionsState
): Promise<boolean> {
...
async function deleteLegacyZedCodeConfig(projectDirectory: string): Promise<void> {
  const legacyPath = getLegacyConfigPath(projectDirectory);
  const runtimeFiles = getRuntimeFilesAPI();
