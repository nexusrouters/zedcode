...
type ZedCodeEvent = ScheduledTaskRanEvent | SessionCreatedEvent | BrowserControlRequestEvent;
type Listener = (event: ZedCodeEvent) => void;
...
const dispatchFromEnvelope = (envelope: { type: string; properties: unknown }) => {
  if (envelope.type === 'zedcode:event-stream-ready') {
    reconnectAttempt = 0;
    return;
  }

  if (envelope.type === 'zedcode:heartbeat') {
    return;
  }

  if (envelope.type === 'zedcode:session-created') {
  ...
  if (envelope.type === 'zedcode:browser-control-request') {
    const properties = getEventProperties(envelope.properties);
  ...
  if (envelope.type !== 'zedcode:scheduled-task-ran') {
    return;
  }
...
const connect = () => {
  ...
  const canControlBrowser = typeof window !== 'undefined' && Boolean(window.__ZEDCODE_ELECTRON__);
...
export const subscribeZedcodeEvents = (listener: Listener): (() => void) => {
