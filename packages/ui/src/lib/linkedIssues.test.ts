...
const sessionWith = (linked: unknown): Session =>
  ({ metadata: { zedcode: { linked_issues: linked } } } as unknown as Session);
...
describe('withLinkedIssue', () => {
  test('adds a link and preserves unrelated metadata', () => {
    const next = withLinkedIssue(
      { zedcode: { kind: 'review' }, other: 1 },
      issue(),
      true,
    );
    expect(next.other).toBe(1);
    expect((next.zedcode as Record<string, unknown>).kind).toBe('review');
    expect((next.zedcode as { linked_issues: LinkedIssue[] }).linked_issues).toEqual([issue()]);
  });
  ...
  test('re-linking replaces the entry rather than duplicating it', () => {
    ...
    const stored = (second.zedcode as { linked_issues: LinkedIssue[] }).linked_issues;
