 ...
 *    `window.__ZEDCODE_HARDWARE_KEYBOARD__` at document start, then keeps it
...
declare global {
  interface Window {
    __ZEDCODE_HARDWARE_KEYBOARD__?: boolean;
  }
}
...
const initialNativeAnswer = typeof window !== 'undefined'
  && typeof window.__ZEDCODE_HARDWARE_KEYBOARD__ === 'boolean'
  ? window.__ZEDCODE_HARDWARE_KEYBOARD__
  : null;
...
export const startHardwareKeyboardBridge = (): (() => void) => {
  ...
  if (typeof window.__ZEDCODE_HARDWARE_KEYBOARD__ === 'boolean') {
    adopt(window.__ZEDCODE_HARDWARE_KEYBOARD__);
  }
