...
export type DesktopSettings = {
  ...
  // User-added skills catalogs (persisted to ~/.config/zedcode/settings.json)
...
const getElectronRuntime = (): ElectronRuntimeGlobal | null => {
  ...
  return (window as unknown as { __ZEDCODE_ELECTRON__?: ElectronRuntimeGlobal }).__ZEDCODE_ELECTRON__ ?? null;
};

const getDesktopBridge = (): DesktopBridgeGlobal | null => {
  if (typeof window === 'undefined') return null;
  return (window as unknown as { __ZEDCODE_DESKTOP__?: DesktopBridgeGlobal }).__ZEDCODE_DESKTOP__ ?? null;
};
...
export const isDesktopLocalOriginActive = (): boolean => {
  ...
  const local = typeof window.__ZEDCODE_LOCAL_ORIGIN__ === 'string' ? window.__ZEDCODE_LOCAL_ORIGIN__ : '';
...
export const getDesktopHomeDirectory = async (): Promise<string | null> => {
  if (typeof window !== 'undefined') {
    const embedded = window.__ZEDCODE_HOME__;
...
export const downloadDesktopUpdate = async (
  ...
  try {
    if (typeof onProgress === 'function' && bridge?.listen) {
      unlisten = await bridge.listen('zedcode:update-progress', (evt) => {
