...
export const getContextObligatoryMessages = (
  ...
  if (!isRecord(zedcode) || !Array.isArray(zedcode.context_obligatory_messages)) return [];

  return zedcode.context_obligatory_messages.filter((value): value is ContextObligatoryMessage =>
    isRecord(value)
...
export const withContextObligatoryMessage = (
  ...
  const current = Array.isArray(zedcode.context_obligatory_messages)
    ? zedcode.context_obligatory_messages.filter((value): value is ContextObligatoryMessage =>
