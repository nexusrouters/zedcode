...
export type DesktopSshInstance = {
  ...
  auth: {
    ...
    zedcodePassword?: DesktopSshStoredSecret;
  };
...
const parseInstance = (value: unknown): DesktopSshInstance | null => {
  ...
  const remoteRaw = isRecord(value.remoteZedcode)
    ? value.remoteZedcode
    : isRecord(value.remote_zedcode)
      ? value.remote_zedcode
      : {};
  ...
  const zedcodePassword = parseStoredSecret(authRaw.zedcodePassword || authRaw.zedcode_password);
  ...
  return {
    ...
    auth: {
      ...
      ...(zedcodePassword ? { zedcodePassword } : {}),
    },
    portForwards,
  };
};
...
export const listenDesktopSshStatus = async (
  ...
  const desktop = (window as unknown as { __ZEDCODE_DESKTOP__?: DesktopBridgeGlobal }).__ZEDCODE_DESKTOP__;
  const listen = desktop?.listen;
  if (typeof listen !== 'function') {
    return async () => {};
  }

  const unlisten = await listen('zedcode:ssh-instance-status', (event) => {
