...
const ANNOTATION_ACTIVE_FLAG = '__zedcodeBrowserAnnotationActive';

/** Cancels an overlay left behind by an earlier session. Safe when none is active. */
export const ANNOTATION_TEARDOWN_SCRIPT = `(() => {
  try {
    var host = document.querySelector('[data-zedcode-annotation]');
    if (host && host.parentNode) host.parentNode.removeChild(host);
  } catch (error) { /* page navigated away */ }
  try {
    var cursor = document.getElementById('zedcode-annotation-cursor');
...
export const buildAnnotationOverlayScript = (
  ...
  var OVERLAY_ATTR = 'data-zedcode-annotation';
  ...
  cursorStyle.id = 'zedcode-annotation-cursor';
