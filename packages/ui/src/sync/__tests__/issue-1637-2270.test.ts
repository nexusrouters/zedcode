...
const markSessionAsZedCodeCreatedCalls: string[] = []
...
mock.module("../session-ui-store", () => ({
  useSessionUIStore: {
    getState: () => ({
      ...
      markSessionAsZedCodeCreated: (sessionId: string) => {
        markSessionAsZedCodeCreatedCalls.push(sessionId)
      },
    }),
  },
}))
...
beforeEach(() => {
  ...
  markSessionAsZedCodeCreatedCalls.length = 0
