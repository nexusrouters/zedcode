...
const normalizeZedCodeSessionStatus = (payload: Event): Event | null => {
  ...
  if (record.type !== "zedcode:session-status") return null
  ...
  return {
    id: typeof record.id === "string" && record.id.length > 0
      ? record.id
      : `zedcode-status-${sessionID}-${Date.now()}`,
    type: "session.status",
    properties: {
      sessionID,
      status,
...
const normalizeEventType = (payload: Event): Event => {
  const normalizedZedCodeStatus = normalizeZedCodeSessionStatus(payload)
  if (normalizedZedCodeStatus) {
    return normalizedZedCodeStatus
  }
...
export function createEventPipeline(input: EventPipelineInput): EventPipeline {
  ...
  if (typeof globalThis.window !== "undefined") {
    globalThis.window.addEventListener("zedcode:system-resume", onSystemResume)
  ...
  const cleanup = () => {
    ...
    if (typeof globalThis.window !== "undefined") {
      globalThis.window.removeEventListener("zedcode:system-resume", onSystemResume)
