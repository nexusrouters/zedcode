const STORAGE_KEY = "zedcode_session_load_perf"
...
declare global {
  interface Window {
    __zedcodeSessionLoadPerformance?: SessionLoadPerformanceState
  }
}
...
export function startSessionLoadPerformanceEvent(input: Omit<SessionLoadPerformanceEvent, "at" | "durationMs" | "outcome">) {
  ...
  return (
    ...
    const state = window.__zedcodeSessionLoadPerformance ?? { events: [] }
    ...
    window.__zedcodeSessionLoadPerformance = state
  }
}
...
const FIRST_VISIBLE_MARK = "zedcode.chat.first_message_visible"
