...
async function resolveTargetPort(options = {}, deps = {}) {
  ...
  if (ports.length > 1) {
    ...
    throw new TunnelCliError(
      `Multiple ZedCode instances are running (ports: ${ports.join(', ')}). Choose one with --port <port>.`,
  ...
  throw new TunnelCliError(
    'No running ZedCode server found. Start one with `zedcode serve`, or pass --port <port>.',
