...
async function getPidFilePath(port) {
  return path.join(getRunDir(), `zedcode-${port}.pid`);
}

async function getInstanceFilePath(port) {
  return path.join(getRunDir(), `zedcode-${port}.json`);
}
...
// that PID to an unrelated process — see isZedcodeProcessRunning.
...
function isZedcodeCmdline(cmdline) {
  if (typeof cmdline !== 'string' || cmdline.length === 0) {
    return false;
  }
  // Every install path contains the "zedcode" segment — the npm package
  // (@zedcode/web) and the source checkout both do, for the foreground
  ...
  return cmdline.toLowerCase().includes('zedcode');
}

// Liveness + identity — "is the ZedCode instance recorded in a pid file
...
function isZedcodeProcessRunning(pid) {
  const state = getZedcodeProcessState(pid);
  return state === 'matched' || state === 'unknown';
}

function getZedcodeProcessState(pid, options = {}) {
  ...
  return isZedcodeCmdline(cmdline) ? 'matched' : 'mismatched';
}

function hasZedcodeRuntimeInfo(info) {
