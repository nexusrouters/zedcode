...
const STARTUP_SERVICE_ID = 'dev.zedcode.web';
...
function getStartupServicePaths() {
  ...
  if (process.platform === 'linux') {
    return {
      platform: 'linux',
      servicePath: path.join(os.homedir(), '.config', 'systemd', 'user', 'zedcode.service'),
    };
  }
...
function getMacosStartupWrapperPath() {
  return path.join(getDataDir(), 'bin', 'ZedCode');
}
...
function collectStartupEnv(options = {}) {
  ...
  if (uiPassword) {
    env.ZEDCODE_UI_PASSWORD = uiPassword;
  }
  if (options.apiOnly === true) {
    env.ZEDCODE_API_ONLY = 'true';
  }
  if (typeof process.env.ZEDCODE_DATA_DIR === 'string' && process.env.ZEDCODE_DATA_DIR.trim().length > 0) {
    env.ZEDCODE_DATA_DIR = path.resolve(process.env.ZEDCODE_DATA_DIR.trim());
  }
  return env;
}
...
function buildMacosLaunchAgent(options = {}) {
  ...
  const env = collectStartupEnv(options);
  const logDir = path.join(os.homedir(), 'Library', 'Logs', 'ZedCode');
...
function getStartupStatus() {
  ...
  if (paths.platform === 'linux') {
    const enabledResult = runStartupCommand('systemctl', ['--user', 'is-enabled', 'zedcode.service'], { allowFailure: true });
    const activeResult = runStartupCommand('systemctl', ['--user', 'is-active', 'zedcode.service'], { allowFailure: true });
...
function enableStartupService(options = {}) {
  ...
  if (paths.platform === 'macos') {
    ...
    fs.mkdirSync(path.join(os.homedir(), 'Library', 'Logs', 'ZedCode'), { recursive: true, mode: 0o700 });
  ...
  if (paths.platform === 'linux') {
    ...
    runStartupCommand('systemctl', ['--user', 'enable', '--now', 'zedcode.service']);
    return getStartupStatus();
  }
...
function disableStartupService() {
  ...
  if (paths.platform === 'linux') {
    runStartupCommand('systemctl', ['--user', 'disable', '--now', 'zedcode.service'], { allowFailure: true });
    try { fs.unlinkSync(paths.servicePath); } catch {}
