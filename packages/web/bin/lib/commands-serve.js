...
import { discoverZedCodeInstanceOnPort, isDesktopRuntimeForPort } from './cli-lifecycle.js';
...
async function serveCommand(options) {
    ...
    if (targetPort !== 0 && !options.suppressUnsafePortWarning) {
      assertSafeBrowserPort(targetPort, { context: 'ZedCode serve' });
    }

    if (targetPort !== 0) {
      const existingInstance = await discoverZedCodeInstanceOnPort(targetPort, { host: effectiveHost });
      if (existingInstance?.runtime === 'desktop') {
        throw new Error(
          `Port ${targetPort} is used by ZedCode Desktop app. Choose another port or stop the desktop app.`
        );
      }
      if (existingInstance) {
        const pidSuffix = Number.isFinite(existingInstance.pid) ? ` (PID: ${existingInstance.pid})` : '';
        if (existingInstance.source === 'probe') {
          throw new Error(`ZedCode is already running on port ${targetPort}. Use \`zedcode status\` or \`zedcode stop --port ${targetPort}\`.`);
        }
        throw new Error(`ZedCode is already running on port ${targetPort}${pidSuffix}`);
      }
    ...
    if (!effectiveUiPassword && !options.suppressUiPasswordWarning) {
      ...
      const warningLine = 'ZEDCODE_UI_PASSWORD is not set';
      const warningDetail = networkExposed
        ? `server is bound to ${bindHost} and reachable on your network with no UI auth. `
          + 'Set --ui-password or ZEDCODE_UI_PASSWORD before exposing it over LAN.'
        : 'browser UI is unsecured. Use --ui-password or ZEDCODE_UI_PASSWORD.';
      if (showOutput) {
    ...
    if (options.foreground) {
      ...
      if (effectiveUiPassword) {
        process.env.ZEDCODE_UI_PASSWORD = effectiveUiPassword;
      }
      process.env.ZEDCODE_HOST = effectiveHost;
      process.env.ZEDCODE_RUNTIME = 'web';
      ...
      if (!isQuietMode(options)) {
        console.log(`Starting ZedCode on port ${targetPort === 0 ? 'auto' : targetPort} (foreground)`);
      }
    ...
    const child = spawn(runtimeBin, serverArgs, {
      ...
      env: {
        ...process.env,
        ZEDCODE_PORT: String(targetPort),
        ZEDCODE_RUNTIME: 'web',
        OPENCODE_BINARY: opencodeBinary,
        ZEDCODE_HOST: effectiveHost,
        ...(effectiveUiPassword ? { ZEDCODE_UI_PASSWORD: effectiveUiPassword } : {}),
        ...(options.apiOnly === true ? { ZEDCODE_API_ONLY: 'true' } : {}),
        ...(process.env.OPENCODE_SKIP_START ? { ZEDCODE_SKIP_OPENCODE_START: process.env.OPENCODE_SKIP_START } : {}),
      },
    });

    child.unref();
    serveSpin?.start(`Starting ZedCode on port ${targetPort === 0 ? 'auto' : targetPort}...`);
    ...
    try {
      resolvedPort = await new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          if (settled) return;
          settled = true;
          reject(new Error(`ZedCode daemon did not report ready within ${DAEMON_READY_TIMEOUT_MS / 1000}s`));
        }, DAEMON_READY_TIMEOUT_MS);

        child.on('message', (msg) => {
          if (settled) return;
          if (msg && msg.type === 'zedcode:ready' && typeof msg.port === 'number') {
            settled = true;
        ...
        child.on('exit', (code, signal) => {
          ...
          settled = true;
          clearTimeout(timeout);
          reject(new Error(`ZedCode daemon exited before reporting ready${signal ? ` (${signal})` : ` (code ${code ?? 'unknown'})`}`));
        });
      });
    } catch (error) {
    ...
    if (!isProcessRunning(child.pid)) {
      serveSpin?.error('Failed to start ZedCode');
    ...
    const serveResult = {
      ...
      logs: `zedcode logs -p ${resolvedPort}`,
      launchMode: 'daemon',
    };
    ...
    if (isQuietMode(options)) {
      ...
      // in the same compact `pass:` token form `zedcode status --quiet`
