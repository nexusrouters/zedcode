...
import {
  ...
  readInstanceOptions,
  removeInstanceFile,
  getZedcodeProcessState,
  hasZedcodeRuntimeInfo,
} from './cli-process.js';
...
function createLivePortInstance(port, info, host) {
  if (!hasZedcodeRuntimeInfo(info)) return null;
  return {
    port,
    pid: Number.isFinite(info.pid) ? info.pid : null,
    pidFilePath: path.join(getRunDir(), `zedcode-${port}.pid`),
    instanceFilePath: path.join(getRunDir(), `zedcode-${port}.json`),
    mtime: 0,
    startedAt: 0,
    launchMode: 'daemon',
...
async function resolveDoctorPortStatuses(options = {}) {
  ...
  if (options.explicitPort) {
    ...
    if (desktopEntry && desktopEntry.port === requestedPort) {
      statuses.push({
        ...
        detail: 'Use a CLI instance port from `zedcode serve` for tunneling.',
      });
    ...
    statuses.push({
      ...
      detail: `Start one with \`zedcode serve --port ${requestedPort}\`.`,
    });
    return { statuses, availableEntries: [] };
  }
  ...
  if (runningEntries.length === 0) {
    statuses.push({
      ...
      detail: 'Start one with `zedcode serve`.',
    });
  }
...
async function discoverRunningInstances(options = {}) {
  ...
  const getProcessState = typeof options.getZedcodeProcessState === 'function'
    ? options.getZedcodeProcessState
    : (pid) => getZedcodeProcessState(pid, options);
  try {
    const files = fs.readdirSync(runDir);
    const pidFiles = files.filter((file) => file.startsWith('zedcode-') && file.endsWith('.pid'));
    for (const file of pidFiles) {
      const port = parseInt(file.replace('zedcode-', '').replace('.pid', ''), 10);
      if (!Number.isFinite(port) || port <= 0) continue;
      const pidFilePath = path.join(runDir, file);
      const pid = readPidFile(pidFilePath);
      if (!pid) {
        removePidFile(pidFilePath);
        removeInstanceFile(path.join(runDir, `zedcode-${port}.json`));
        continue;
      }

      const instanceFilePath = path.join(runDir, `zedcode-${port}.json`);
      ...
      // confirms ZedCode. Cmdline identity alone can match a recycled PID
      // from another ZedCode process on a different port. Try all plausible
      ...
      if (!hasZedcodeRuntimeInfo(liveInfo)) {
...
async function discoverZedCodeInstanceOnPort(port, options = {}) {
...
async function discoverLifecycleInstances(options = {}, deps = {}) {
  ...
  const liveInstance = await discoverZedCodeInstanceOnPort(options.port, {
    ...deps,
    host: options.host,
    runningInstances,
  });
...
async function discoverUnconfirmedRegistryInstanceOnPort(port, options = {}) {
  ...
  const storedOptions = readInstanceOptions(instanceFilePath);
  const processState = getZedcodeProcessState(pid);
  if (processState === 'dead') {
    removePidFile(pidFilePath);
    removeInstanceFile(instanceFilePath);
    return null;
