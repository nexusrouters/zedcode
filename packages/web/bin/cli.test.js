...
import {
  ...
  getPidFilePath,
  isZedcodeCmdline,
  isZedcodeProcessRunning,
  parseArgs,
  resolveServeHost,
  resolveServeUiPassword,
} from './cli.js';

async function withTempZedCodeDataDir(fn) {
  const previous = process.env.ZEDCODE_DATA_DIR;
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zedcode-cli-test-'));
  process.env.ZEDCODE_DATA_DIR = dir;
  try {
    return await fn(dir);
  } finally {
    if (typeof previous === 'string') {
      process.env.ZEDCODE_DATA_DIR = previous;
    } else {
      delete process.env.ZEDCODE_DATA_DIR;
    }
...
async function startMockZedCodeServer(options = {}) {
...
function spawnZedCodeLikeIdleProcess() {
  return spawn(process.execPath, ['-e', 'setInterval(() => {}, 1000)', 'zedcode-idle'], { stdio: 'ignore' });
}

function spawnZedCodeLikeHungServer(port) {
  const script = `
    ...
    setInterval(() => {}, 1000);
  `;
  return spawn(process.execPath, ['-e', script, 'zedcode-hung-server'], { stdio: 'ignore' });
}
...
describe('cli args', () => {
  ...
  it('parses explicit connect-url server overrides', () => {
    const parsed = parseArgs(['connect-url', '--server', 'https://zedcode.example.com', '--port', '3002']);

    expect(parsed.command).toBe('connect-url');
    expect(parsed.options.server).toBe('https://zedcode.example.com');
  ...
  it('parses session create options', () => {
    ...
    expect(parsed.options.branch).toBe('zedcode/side-task');
  ...
  it('formats projects compactly', () => {
    expect(formatProjectLine({
      ...
      label: 'Zedcode',
      path: '/repo/zedcode',
    })).toBe('- `Zedcode` — `path_repo` — `/repo/zedcode`');
  });
...
describe('cli API target resolution', () => {
  ...
  it('fails when multiple non-default instances are running', async () => {
    await expect(resolveTargetPort({}, {
      ...
      isServerHealthReady: async () => false,
    })).rejects.toThrow('Multiple ZedCode instances are running');
  });
});
...
describe('network-exposed auth validation', () => {
  ...
  it('allows explicit unsafe LAN override from process env only', () => {
    const previous = process.env.ZEDCODE_ALLOW_UNAUTHENTICATED_LAN;
    process.env.ZEDCODE_ALLOW_UNAUTHENTICATED_LAN = 'true';
    try {
      expect(() => assertAuthenticatedNetworkExposure({ host: '0.0.0.0' })).not.toThrow();
    } finally {
      if (typeof previous === 'string') {
        process.env.ZEDCODE_ALLOW_UNAUTHENTICATED_LAN = previous;
      } else {
        delete process.env.ZEDCODE_ALLOW_UNAUTHENTICATED_LAN;
      }
    }
  });
});
...
describe('serve host resolution', () => {
  it('uses ZEDCODE_HOST when --host is not provided', () => {
    const previous = process.env.ZEDCODE_HOST;
    process.env.ZEDCODE_HOST = '192.0.2.20';
    try {
      expect(resolveServeHost(undefined)).toBe('192.0.2.20');
    } finally {
      if (typeof previous === 'string') {
        process.env.ZEDCODE_HOST = previous;
      } else {
        delete process.env.ZEDCODE_HOST;
      }
    }
  });

  it('prefers explicit --host over ZEDCODE_HOST', () => {
    const previous = process.env.ZEDCODE_HOST;
    process.env.ZEDCODE_HOST = '192.0.2.20';
    try {
...
describe('compatibility exports', () => {
  it('allows tunnel profile migration before command options are initialized', async () => {
    await withTempZedCodeDataDir(async () => {
...
describe('CLI HTTP helpers', () => {
  it('sends one typed request to the shared control endpoint', async () => {
    ...
    globalThis.fetch = async (url, options = {}) => {
      expect(new URL(String(url)).pathname).toBe('/api/zedcode/control');
  ...
  it('retries UI-authenticated API requests with the stored instance password', async () => {
    await withTempZedCodeDataDir(async () => {
      ...
      try {
        const { response, body } = await requestJson(port, '/api/zedcode/tunnel/start', {
          method: 'POST',
  ...
  it('prefers the stored instance password over a non-explicit env password', async () => {
    await withTempZedCodeDataDir(async () => {
      ...
      try {
        const { response, body } = await requestJson(port, '/api/zedcode/scheduled-tasks/status', {
  ...
  it('authenticates desktop-local API requests with the stored client token', async () => {
    await withTempZedCodeDataDir(async (dir) => {
      const port = 57123;
...
describe('cli entry detection', () => {
  const modulePath = '/tmp/zedcode/bin/cli.js';
  const moduleUrl = pathToFileURL(modulePath).href;

  it('resolves symlinked entry paths before comparing', () => {
    const symlinkPath = '/usr/local/bin/zedcode';
  ...
  it('accepts wrapper binary name fallback when requested', () => {
    const wrapperPath = '/home/user/.local/bin/zedcode';
    expect(isModuleCliExecution(wrapperPath, moduleUrl, undefined, 'zedcode')).toBe(true);
  });
...
describe('isZedcodeCmdline', () => {
  it('accepts ZedCode CLI and daemon cmdlines', () => {
    expect(isZedcodeCmdline('node /x/@zedcode/web/bin/cli.js serve')).toBe(true);
    expect(isZedcodeCmdline('node /x/@zedcode/web/server/index.js --port 9090')).toBe(true);
    expect(isZedcodeCmdline('bun /home/u/projects/zedcode/packages/web/server/index.js --port 3001')).toBe(true);
  });

  it('rejects recycled and unrelated processes (issue #1721)', () => {
    expect(isZedcodeCmdline('node /home/herjarsa/npm-global/bin/agentmemory')).toBe(false);
    expect(isZedcodeCmdline('node /usr/lib/node_modules/npm/bin/npm-cli.js install')).toBe(false);
    expect(isZedcodeCmdline('')).toBe(false);
    expect(isZedcodeCmdline(null)).toBe(false);
  });
});

describe('isZedcodeProcessRunning', () => {
  it('returns false for a dead PID', () => {
    expect(isZedcodeProcessRunning(2147483646)).toBe(false);
  });
  ...
  it.skipIf(process.platform !== 'linux' && process.platform !== 'darwin')(
    'returns false for a live non-ZedCode PID',
    async () => {
      const child = spawn('sleep', ['30'], { stdio: 'ignore' });
      try {
        await new Promise((resolve) => setTimeout(resolve, 150));
        expect(isZedcodeProcessRunning(child.pid)).toBe(false);
      } finally {
        child.kill('SIGKILL');
      }
    }
...
describe('lifecycle instance discovery', () => {
  it('does not attribute a desktop runtime response to a different explicit port', async () => {
    await withTempZedCodeDataDir(async (dir) => {
      ...
      const instance = await discoverZedCodeInstanceOnPort(3003, {
  ...
  it('attributes a desktop runtime response to its configured desktop port', async () => {
    await withTempZedCodeDataDir(async (dir) => {
      ...
      const instance = await discoverZedCodeInstanceOnPort(57123, {
        fetchImpl: async () => createMockJsonResponse({ runtime: 'desktop', pid: 934 }),
      });
  ...
  it('keeps pid and instance files when live port probe confirms a cmdline mismatch', async () => {
    await withTempZedCodeDataDir(async () => {
      ...
      const instances = await discoverRunningInstances({
        ...
        getZedcodeProcessState: () => 'mismatched',
      });
  ...
  it('preserves matched pid and instance files when the recorded port probe is inconclusive', async () => {
    await withTempZedCodeDataDir(async () => {
      ...
      const instances = await discoverRunningInstances({
        fetchImpl: async () => createMockJsonResponse(null, false),
        getZedcodeProcessState: () => 'matched',
      });
  ...
  it('preserves unknown-identity pid and instance files when the recorded port probe is inconclusive', async () => {
    await withTempZedCodeDataDir(async () => {
      ...
      const instances = await discoverRunningInstances({
        fetchImpl: async () => createMockJsonResponse(null, false),
        getZedcodeProcessState: () => 'unknown',
      });
  ...
  it('uses the live system-info pid instead of a stale ZedCode-looking pid-file pid', async () => {
    await withTempZedCodeDataDir(async () => {
      const port = 45127;
  ...
  it('discovers an explicit live ZedCode port without a pid-file registry entry', async () => {
    await withTempZedCodeDataDir(async () => {
      const port = 45125;
      const instances = await discoverLifecycleInstances(
  ...
  it('cleans a matched pid-file entry without stopping it when the recorded port is free', async () => {
    await withTempZedCodeDataDir(async () => {
      const port = await allocateLoopbackPort();
      const child = spawnZedCodeLikeIdleProcess();
      const pidFile = await getPidFilePath(port);
      const instanceFile = await getInstanceFilePath(port);
      try {
        await new Promise((resolve) => setTimeout(resolve, 150));
...
describe('lifecycle commands with unmanaged explicit ports', () => {
  it('serve refuses to start on a live ZedCode port without requiring pid files', async () => {
    await withTempZedCodeDataDir(async () => {
      const server = await startMockZedCodeServer();
      try {
  ...
  it('stop --port can recover a matched pid-file instance whose HTTP endpoint is unresponsive', async () => {
    await withTempZedCodeDataDir(async () => {
      const port = await allocateLoopbackPort();
      const child = spawnZedCodeLikeHungServer(port);
      const pidFile = await getPidFilePath(port);
      const instanceFile = await getInstanceFilePath(port);
      try {
  ...
  it('plain stop ignores a stale CLI registry entry that resolves to desktop runtime', async () => {
    await withTempZedCodeDataDir(async () => {
      const server = await startMockZedCodeServer({ runtime: 'desktop' });
      const child = spawn('sleep', ['30'], { stdio: 'ignore' });
