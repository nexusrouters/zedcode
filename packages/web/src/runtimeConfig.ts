import { getRuntimeExtraHeadersSync, refreshLocalRuntimeUrlAuthToken, refreshRuntimeUrlAuthToken, setRuntimeBearerToken, setRuntimeExtraHeaders } from '@zedcode/ui/lib/runtime-auth';
import { installRuntimeFetchBridge } from '@zedcode/ui/lib/runtime-fetch';
...
import { configureRuntimeUrlResolver } from '@zedcode/ui/lib/runtime-url';
import type { EmbeddedSessionRuntimeBootstrap } from '@zedcode/ui/components/layout/contextPanelEmbeddedChat';
...
declare global {
  interface Window {
    ...
    __ZEDCODE_RELAY_HOST_ID__?: string;
  }
}
...
export const readRuntimeBootstrapConfig = (): EmbeddedSessionRuntimeBootstrap => {
  ...
  return {
    apiBaseUrl: readString(window.__ZEDCODE_API_BASE_URL__),
    clientToken: readString(window.__ZEDCODE_CLIENT_TOKEN__),
    localOrigin: readString(window.__ZEDCODE_LOCAL_ORIGIN__),
    runtimeHeaders: window.__ZEDCODE_RUNTIME_HEADERS__,
    relayHostId: readString(window.__ZEDCODE_RELAY_HOST_ID__),
  };
};
