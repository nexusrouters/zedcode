...
- This module owns GitHub auth, Octokit access, repo resolution, and Pull Request status resolution for ZedCode.
...
- Auth storage: `~/.config/zedcode/github-auth.json`
- Writes are atomic and file mode is `0o600`.
- Client ID resolution order: `ZEDCODE_GITHUB_CLIENT_ID` -> `settings.json` -> default.
- Scope resolution order: `ZEDCODE_GITHUB_SCOPES` -> `settings.json` -> default.
...
- PR state is persisted in local storage under `zedcode.github-pr-status`.
