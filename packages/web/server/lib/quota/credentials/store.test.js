...
const previousDataDir = process.env.ZEDCODE_DATA_DIR;
const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'zedcode-quota-store-'));
process.env.ZEDCODE_DATA_DIR = temporaryDirectory;
...
afterAll(() => {
  if (previousDataDir === undefined) delete process.env.ZEDCODE_DATA_DIR;
  else process.env.ZEDCODE_DATA_DIR = previousDataDir;
