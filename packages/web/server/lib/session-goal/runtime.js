...
// (metadata.zedcode.goal). While the goal is active, the server keeps the
...
const ZEDCODE_SETTINGS_FILE = path.join(
  process.env.ZEDCODE_DATA_DIR
...
const isSessionGoalEnabled = () => {
  try {
    const raw = fs.readFileSync(ZEDCODE_SETTINGS_FILE, 'utf8');
...
const parseGoalMetadata = (session) => {
  ...
  const namespace = metadata.zedcode;
...
export const createSessionGoalRuntime = ({
  ...
  const writeGoal = async (sessionId, directory, expectedGoalId, mutate) => {
    ...
    const currentNamespace = currentMetadata.zedcode && typeof currentMetadata.zedcode === 'object'
      ? currentMetadata.zedcode
      : {};
    await openCodeFetch(`/session/${encodeURIComponent(sessionId)}`, {
      ...
      body: {
        metadata: {
          ...currentMetadata,
          zedcode: { ...currentNamespace, goal: nextGoal },
        },
      },
    });
    return nextGoal;
  ...
  const tick = async (sessionId, directory) => {
    ...
    // TEXT lives under the ZedCode data dir keyed by session id and is
