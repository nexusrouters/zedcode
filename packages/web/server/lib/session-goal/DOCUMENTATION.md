...
objective stored under `metadata.zedcode.goal`, with the small model as
an independent progress auditor. Built on ZedCode's backend-driven
architecture (session-assist is the structural template): the loop lives in
the web server and survives UI disconnects.

## Goal payload (`metadata.zedcode.goal`)
...
`ZEDCODE_DATA_DIR` or `~/.config/zedcode`), keyed by the SESSION ID:
...
- `routes.js` — `PUT/GET/DELETE /api/goals/objective/:sessionId`
  (ZedCode-owned, registered before the generic proxy; JSON parsing via
...
stamp `metadata.zedcode.goal` onto the fresh session (objective = the
...
`zedcode session create --prompt <text> --goal` uses the explicit
`POST /api/zedcode/sessions` orchestration route. The server creates the
...
`zedcode session send --goal` and `zedcode session fork --goal` use
