...
// ---------------------------------------------------------------------------
// Regression for https://github.com/zedcode/zedcode/issues/2746
//
// "[Bug] new worktree， Filename too long"
//
// ZedCode places worktrees under:
...
const createTempDir = () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zedcode-git-issue2746-'));
  tempDirs.push(dir);
  return dir;
};
  ...
  it('surfaces guided bootstrap failure when a path component exceeds the filesystem name limit', async () => {
    ...
    try {
      ...
      const created = await createWorktree(repo, {
        ...
        branchName: 'zedcode/issue-2746-namemax',
      });
      expect(created.directoryCreated).toBe(true);
