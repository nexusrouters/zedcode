...
const createGit = async (directory, { allowUnsafeSshCommand = false } = {}) => {
  ...
  // makes simple-git use process.cwd(), which breaks when the ZedCode
...
// ZedCode places managed worktrees under a deep data-dir path
...
const formatWorktreePopulateError = (message) => {
  ...
  return [
    ...
    'ZedCode enables Git `core.longpaths` for worktree population; if this still fails on Windows, enable OS long paths (LongPathsEnabled) or open the repository from a shorter absolute path.',
  ].join('\n');
};
...
export const ensureWorktreeLongpaths = async (directory) => {
  ...
  // subsequent ZedCode and CLI git operations in this repo also get long
...
const createIntegrateTempWorktree = async (repoRoot, targetBranch) => {
  const tmpParent = path.join(os.homedir(), '.config', 'zedcode', 'tmp');
...
// whose directory no longer exists when reading them back. ZedCode used to
...
const writeTempPatchFile = async (patch) => {
  ...
  const tmpPath = path.join(tmpDir, `zedcode-hunk-${Date.now()}-${Math.random().toString(36).slice(2)}.patch`);
...
export async function stashPush(directory, options = {}) {
  ...
  const message = typeof options.message === 'string' && options.message.trim()
    ...
    : `ZedCode stash ${new Date().toISOString()}`;
