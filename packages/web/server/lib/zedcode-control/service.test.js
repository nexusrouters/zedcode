...
import { createZedCodeControlService } from './service.js';
...
const createService = (overrides = {}) => {
  ...
  const service = createZedCodeControlService({
...
describe('ZedCode control service', () => {
  ...
  it('rejects actions outside the fixed contract', async () => {
    const { service } = createService();
    await expect(service.execute('session.delete')).rejects.toThrow('Unsupported ZedCode action');
  });
});
...
describe('browser capture', () => {
  ...
  it('saves the image beside the code and hands back a path the answer can use', async () => {
    ...
    expect(result.path.startsWith('.zedcode/screenshots/after-fix-')).toBe(true);
