...
const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zedcode-ui-auth-test-'));
process.env.ZEDCODE_DATA_DIR = dataDir;
...
describe('ui auth client credential seam', () => {
  ...
  it('issues desktop client tokens with the UI session expiry', async () => {
    ...
    expect(createClientInput.label).toBe('ZedCode Desktop');
