...
const { registerZedCodeRoutes } = await import('./zedcode-routes.js');
...
const createApp = ({ environment = {}, storedOptions = {} } = {}) => {
  ...
  const dependencies = {
    ...
    __dirname: '/opt/zedcode/server',
    zedcodeDataDir: '/tmp/zedcode',
  ...
  registerZedCodeRoutes(app, dependencies);
...
beforeEach(() => {
  ...
  packageManager.getUpdateCommand.mockReturnValue('npm install -g @zedcode/web@latest');
});
...
describe('ZedCode foreground update route', () => {
  ...
  it('rejects an unsafe systemd unit override before starting an update job', async () => {
    const { app } = createApp({
      environment: {
        ...
        ZEDCODE_SYSTEMD_UNIT: 'zedcode.service; rm -rf /',
      },
    });
  ...
  it('queues the install in a transient systemd unit and returns its job identifier', async () => {
    ...
    const { app } = createApp({
      environment: {
        INVOCATION_ID: 'systemd-invocation',
        ZEDCODE_SYSTEMD_UNIT: 'zedcode@wsl.service',
    ...
    await request(app)
      ...
      .expect(200, {
        ...
        jobId: 'zedcode-update-1700000000000',
        logPath: 'journalctl --user-unit zedcode-update-1700000000000.service',
      });

    expect(childProcess.spawnSync).toHaveBeenCalledWith('systemd-run', [
      '--user',
      '--unit=zedcode-update-1700000000000',
      '--collect',
      '--service-type=exec',
      '--setenv=PATH=/home/syu/.npm-global/bin:/usr/bin:/bin',
      '/bin/sh',
      '-c',
      "set -eu\nnpm install -g @zedcode/web@latest\nsystemctl --user restart 'zedcode@wsl.service'",
    ], {
      encoding: 'utf8',
