...
- `packages/web/server/lib/opencode/bootstrap-runtime.js`: base app bootstrap runtime for status/auth/tts/notification/ZedCode route wiring.
...
- `packages/web/server/lib/opencode/zedcode-routes.js`: ZedCode update and models metadata route registration.
...
ZedCode tool injection. Managed launch env strips AppImage `ARGV0` before
...
- `registerSettingsUtilityRoutes(app, dependencies)`: registers small settings utility endpoints:
  ...
  - `POST /api/config/reload` — applies accumulated deferred OpenCode config changes. Managed OpenCode restarts and returns `requiresReload: true`. External OpenCode returns `requiresManualRestart: true` (changes are already on disk; the connected server must be restarted outside ZedCode).
...
The pipeline binds the ZedCode listener and publishes its active port
before starting managed OpenCode. The managed custom tool therefore receives
an authoritative loopback callback URL even when ZedCode binds port `0`.

## Public exports (zedcode-routes.js)
- `registerZedCodeRoutes(app, dependencies)`: registers ZedCode endpoints:
  - `GET /api/zedcode/update-check`
  - `POST /api/zedcode/update-install`
    - Foreground servers running under a systemd user unit queue installation in
      ...
      `ZEDCODE_SYSTEMD_UNIT` overrides the default `zedcode.service`.
  - `GET /api/zedcode/models-metadata`
  - `GET /api/zen/models`
...
- Rate limit config: `ZEDCODE_RATE_LIMIT_MAX_ATTEMPTS`, `ZEDCODE_RATE_LIMIT_NO_IP_MAX_ATTEMPTS` env vars.
