...
export const createStaticRoutesRuntime = (dependencies) => {
  ...
  const resolveDistPath = () => {
    const env = typeof process.env.ZEDCODE_DIST_DIR === 'string' ? process.env.ZEDCODE_DIST_DIR.trim() : '';
    if (env) {
  ...
  const registerApiOnlyFallbackRoutes = (app) => {
    app.get(/^(?!\/api|\/auth|\/health|.*\.(js|css|svg|png|jpg|jpeg|gif|ico|woff|woff2|ttf|eot|map)).*$/, (req, res) => {
      const command = 'zedcode connect-url --help';
...
<head>
  ...
  <title>ZedCode API-only mode</title>
  <style>
    :root {
      color-scheme: dark;
...
<body>
  <main>
    <svg class="logo" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ZedCode logo">
    ...
    <h1>ZedCode is running in headless mode</h1>
    <p>This server is ready. Open it from the ZedCode desktop or mobile app to use it.</p>
    <div class="command">
  ...
  <script>
    ...
    button?.addEventListener('click', async () => {
      ...
      try {
        ...
        json: () => {
          res.json({ ok: true, mode: 'api-only', message: 'ZedCode is running in API-only mode' });
        },
        default: () => {
          res.type('text/plain').send('ZedCode is running in API-only mode');
        },
      });
    });
  };
