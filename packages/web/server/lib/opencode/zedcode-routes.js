...
function resolveSystemdServiceUnit(environment) {
  ...
  const configuredUnit = typeof environment.ZEDCODE_SYSTEMD_UNIT === 'string'
    ? environment.ZEDCODE_SYSTEMD_UNIT.trim()
    : '';
  const unit = configuredUnit || 'zedcode.service';
...
export const registerZedCodeRoutes = (app, dependencies) => {
  const {
    fs,
    path,
    process,
  ...
  app.get('/api/zedcode/update-check', async (req, res) => {
    try {
  ...
  app.post('/api/zedcode/update-install', async (_req, res) => {
    try {
      ...
      const instanceFilePath = path.join(zedcodeDataDir, 'run', `zedcode-${currentPort}.json`);
      ...
      if (isForegroundService) {
        if (!systemdServiceUnit) {
          return res.status(409).json({
            error: 'Foreground servers must be updated by their service manager. Set ZEDCODE_SYSTEMD_UNIT when running under systemd, or run zedcode update and restart the service.',
          });
        }

        const updateJobName = `zedcode-update-${Date.now()}`;
        ...
        return res.json({
          success: true,
          message: 'Update queued; ZedCode will restart after installation completes',
          version: updateInfo.version,
          packageManager: pm,
          autoRestart: true,
      ...
      let restartCmdFallback = `zedcode serve --port ${storedOptions.port}`;
      ...
      const updateLogPath = path.join(zedcodeDataDir, 'update-install.log');
      const logPreamble = [
        '',
        `=== ZedCode update ${new Date().toISOString()} ===`,
      ...
      res.json({
        ...
        setTimeout(() => {
          ...
          const script = isWindows
            ...
            ${updateCmd}
            if %ERRORLEVEL% EQU 0 (
              echo Update successful, restarting ZedCode...
              ${restartCmd || 'echo Service manager will restart ZedCode.'}
            ) else (
              echo Update failed
              exit /b 1
            )
  ...
  app.get('/api/zedcode/models-metadata', async (_req, res) => {
    try {
