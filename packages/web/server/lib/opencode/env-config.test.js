...
describe('resolveOpenCodeEnvConfig hostname', () => {
  ...
  it('reads ZEDCODE_OPENCODE_HOSTNAME', () => {
    const result = resolveOpenCodeEnvConfig({ env: { ZEDCODE_OPENCODE_HOSTNAME: '0.0.0.0' } });
    expect(result.configuredOpenCodeHostname).toBe('0.0.0.0');
  });

  it('trims surrounding whitespace', () => {
    const result = resolveOpenCodeEnvConfig({ env: { ZEDCODE_OPENCODE_HOSTNAME: '  tailscale-host  ' } });
    expect(result.configuredOpenCodeHostname).toBe('tailscale-host');
  });

  it('warns and falls back for an empty value', () => {
    const logger = { warn: vi.fn(), error: vi.fn() };
    const result = resolveOpenCodeEnvConfig({ env: { ZEDCODE_OPENCODE_HOSTNAME: '   ' }, logger });
  ...
  it('rejects invalid values with a clear error and falls back to loopback', () => {
    ...
    const result = resolveOpenCodeEnvConfig({
      env: { ZEDCODE_OPENCODE_HOSTNAME: 'http://nope:4096' },
      logger,
    });
    expect(result.configuredOpenCodeHostname).toBe('127.0.0.1');
    expect(logger.error).toHaveBeenCalledWith(
      expect.stringContaining('Rejecting ZEDCODE_OPENCODE_HOSTNAME'),
    );
  ...
  it('keeps other env config intact when the hostname is validated', () => {
    const result = resolveOpenCodeEnvConfig({
      env: { ZEDCODE_OPENCODE_HOSTNAME: '0.0.0.0', OPENCODE_PORT: '4096' },
    });
    expect(result.configuredOpenCodeHostname).toBe('0.0.0.0');
