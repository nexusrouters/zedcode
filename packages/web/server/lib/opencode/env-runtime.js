...
export const createOpenCodeEnvRuntime = (deps) => {
  ...
  const bundledOpenCodeCliCandidates = () => {
    ...
    const roots = [
      process.env.ZEDCODE_BUNDLED_OPENCODE_CLI_DIR,
  ...
  const resolveNodeCliPath = () => {
    const explicit = [process.env.NODE_BINARY, process.env.ZEDCODE_NODE_BINARY]
  ...
  const resolveBunCliPath = () => {
    const explicit = [process.env.BUN_BINARY, process.env.ZEDCODE_BUN_BINARY]
      .map((v) => (typeof v === 'string' ? v.trim() : ''))
      .filter(Boolean);
  ...
  const createConfiguredOpencodeBinaryError = (raw, normalized) => {
    ...
    const messageSuffix = 'ZedCode needs the standalone opencode CLI. Install it and set settings.opencodeBinary to the CLI path, for example ~/.opencode/bin/opencode, or leave the setting empty to use PATH lookup.';
    const error = (() => {
  ...
  const createConfiguredWslOpencodeError = (raw) => new Error(
    `Configured settings.opencodeBinary uses WSL but ZedCode could not resolve a WSL OpenCode command: ${raw}. Ensure WSL is available and opencode is installed in the configured distro.`
  );
  ...
  const applyOpencodeBinaryFromSettings = async (options = {}) => {
    ...
    try {
      ...
      if (explicitWslPath && explicitWslPath[1] && explicitWslPath[1].trim().length > 0) {
        ...
        console.warn(`Configured settings.opencodeBinary uses WSL, which is no longer supported by ZedCode desktop: ${raw}`);
        return null;
      }
      ...
      if (process.platform === 'win32' && (isWslExecutableValue(raw) || isWslExecutableValue(normalized || ''))) {
        ...
        console.warn(`Configured settings.opencodeBinary points to WSL, which is no longer supported by ZedCode desktop: ${raw}`);
        return null;
      }
  ...
  const resolveGitBinaryForSpawn = () => {
    ...
    const explicit = [process.env.GIT_BINARY, process.env.ZEDCODE_GIT_BINARY]
      .map((value) => (typeof value === 'string' ? value.trim() : ''))
      .filter(Boolean);
    for (const candidate of explicit) {
      if (isExecutable(candidate)) {
