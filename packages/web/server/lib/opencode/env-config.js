...
export const resolveOpenCodeEnvConfig = (options = {}) => {
  ...
  const configuredOpenCodePort = (() => {
    const raw =
      env.OPENCODE_PORT ||
      env.ZEDCODE_OPENCODE_PORT ||
      env.ZEDCODE_INTERNAL_PORT;
    if (!raw) {
      return null;
    }
  ...
  const configuredOpenCodeHostname = (() => {
    const raw = env.ZEDCODE_OPENCODE_HOSTNAME;
    ...
    if (!trimmed) {
      logger.warn(
        `[config] Ignoring ZEDCODE_OPENCODE_HOSTNAME=${JSON.stringify(raw)}: empty after trimming`,
      );
      return '127.0.0.1';
    }
    if (!isValidOpenCodeHostname(trimmed)) {
      logger.error(
        `[config] Rejecting ZEDCODE_OPENCODE_HOSTNAME=${JSON.stringify(raw)}: `
