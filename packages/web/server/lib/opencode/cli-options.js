export const parseServeCliOptions = ({
  ...
  const envPassword =
    env.ZEDCODE_UI_PASSWORD ||
    env.OPENCODE_UI_PASSWORD ||
    null;
  const envCfTunnel = env.ZEDCODE_TRY_CF_TUNNEL === 'true';
  const envTunnelProvider = env.ZEDCODE_TUNNEL_PROVIDER || undefined;
  const envTunnelMode = env.ZEDCODE_TUNNEL_MODE || undefined;
  const envTunnelConfigRaw = env.ZEDCODE_TUNNEL_CONFIG;
  const envTunnelConfig = typeof envTunnelConfigRaw === 'string'
    ? (envTunnelConfigRaw.trim().length > 0 ? envTunnelConfigRaw.trim() : null)
    : undefined;
  const envTunnelToken = env.ZEDCODE_TUNNEL_TOKEN || undefined;
  const envTunnelHostname = env.ZEDCODE_TUNNEL_HOSTNAME || undefined;
  const envApiOnly = env.ZEDCODE_API_ONLY === '1' || env.ZEDCODE_API_ONLY === 'true';
