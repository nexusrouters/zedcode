...
export const isUnsafeUnauthenticatedLanAllowed = (env = process.env) =>
  env?.ZEDCODE_ALLOW_UNAUTHENTICATED_LAN === 'true';

export const getUnauthenticatedLanErrorMessage = (host) =>
  `ZedCode refuses to bind to ${host || 'a network-exposed host'} without UI authentication. `
  + 'Set --ui-password or ZEDCODE_UI_PASSWORD before exposing it over LAN, '
  + 'or set ZEDCODE_ALLOW_UNAUTHENTICATED_LAN=true to accept the risk.';
