...
describe('checkForUpdates', () => {
  ...
  it('returns available=false when npm only has a prerelease of the current version', async () => {
    fetchMock
      .when('api.zedcode.dev', Promise.reject(new Error('Network error')))
      .when('registry.npmjs.org', {
        ok: true,
        json: async () => ({
  ...
  it('resolves an Android APK asset when the update API returns an AAB', async () => {
    fetchMock
      .when('api.zedcode.dev', {
        ...
        json: async () => ({
          latestVersion: '1.10.0',
          updateAvailable: true,
          downloadUrl: 'https://github.com/zedcode/zedcode/releases/download/v1.10.0/ZedCode-1.10.0-42-android.aab',
        }),
      })
      .when('api.github.com/repos/zedcode/zedcode/releases/tags/v1.10.0', {
        ok: true,
        json: async () => ({
          assets: [
            {
              name: 'ZedCode-1.10.0-42-android.aab',
              browser_download_url: 'https://downloads.example/ZedCode-1.10.0-42-android.aab',
            },
            {
            ...
            {
              name: 'ZedCode-1.10.0-42-android.apk',
              browser_download_url: 'https://downloads.example/ZedCode-1.10.0-42-android.apk',
            },
          ],
        }),
      });
    ...
    expect(result.downloadUrl).toBe('https://downloads.example/ZedCode-1.10.0-42-android.apk');
  });

  it('keeps a direct Android APK URL from the update API', async () => {
    const apkUrl = 'https://github.com/zedcode/zedcode/releases/download/v1.10.0/ZedCode-1.10.0-42-android.apk';
    fetchMock.when('api.zedcode.dev', {
      ok: true,
      json: async () => ({
        latestVersion: '1.10.0',
        updateAvailable: true,
