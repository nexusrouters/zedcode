...
export const createPushRuntime = (deps) => {
  ...
  const resolveVapidSubject = async () => {
    const configured = process.env.ZEDCODE_VAPID_SUBJECT;
    if (typeof configured === 'string' && configured.trim().length > 0) {
      return configured.trim();
    }

    const originEnv = process.env.ZEDCODE_PUBLIC_ORIGIN;
    if (typeof originEnv === 'string' && originEnv.trim().length > 0) {
      const trimmed = originEnv.trim();
      if (isLoopbackHttpOrigin(trimmed)) {
        return 'mailto:zedcode@localhost';
      }
      return trimmed;
    }
  ...
  const ensurePushInitialized = async () => {
    ...
    if (subject === 'mailto:zedcode@localhost') {
      console.warn('[Push] No public origin configured for VAPID; set ZEDCODE_VAPID_SUBJECT or enable push once from a real origin.');
    }
