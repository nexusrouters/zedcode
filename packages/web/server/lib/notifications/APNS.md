...
4. The **relay** (`zedcode-website/apps/api`, Cloudflare Worker) verifies the signature +
...
- **Relay (default):** server has no Apple key; `ZEDCODE_PUSH_RELAY_URL` defaults to
  `https://api.zedcode.dev/v1/push/send` (register URL is derived as `…/register-token`).
- **Direct (fallback):** set `ZEDCODE_PUSH_RELAY_DISABLED=true` + `ZEDCODE_APNS_KEY_ID/
  TEAM_ID/P8` to sign+send from the server itself (HTTP/2 + ES256 JWT); no relay binding needed.

## Config

Server (`apns-runtime.js`):
- `ZEDCODE_PUSH_RELAY_URL` (default the public relay), `ZEDCODE_APNS_ENVIRONMENT`
...
- Direct fallback: `ZEDCODE_APNS_KEY_ID`, `ZEDCODE_APNS_TEAM_ID`, `ZEDCODE_APNS_P8`
  (or `_P8_PATH`), `ZEDCODE_APNS_BUNDLE_ID`, `ZEDCODE_PUSH_RELAY_DISABLED=true`.
...
1. Apple **Keys** (not Certificates) → create an **APNs Auth Key** (`.p8`) → Key ID + Team ID;
   enable **Push Notifications** on App ID `com.zedcode.app`.
2. In the **zedcode-website** repo → Actions secrets: `APNS_P8` (PEM), `APNS_KEY_ID`,
