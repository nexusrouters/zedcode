...
This module injects an opt-in OpenCode plugin only when ZedCode launches
...
1. Settings persist `optimizeSystemPrompt` in ZedCode's `settings.json`.
2. The setting is applied when managed OpenCode restarts.
3. The runtime materializes the plugin under
   `<zedcode-data-dir>/system-prompt/` and appends its `file://` URL to
