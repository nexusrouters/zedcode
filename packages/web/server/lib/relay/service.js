...
// client, and the /api/zedcode/relay/* management routes.
//
...
// Routes are registered with the other ZedCode feature routes, before the
...
export const DEFAULT_RELAY_URL = 'wss://relay.zedcode.dev/ws';
...
const envRelayUrlOverride = () => {
  const raw = process.env.ZEDCODE_RELAY_URL;
...
export const createRelayService = ({
  ...
  const readConfig = async () => {
    ...
    return {
      ...
      // True when the endpoint is pinned by ZEDCODE_RELAY_URL (a self-hosted
  ...
  const standbyStatus = (holderPid) => ({
    state: 'standby',
    lastError: `relay host is owned by another local ZedCode process (pid ${holderPid})`,
    connectedClients: 0,
  });
  ...
  const registerRoutes = (app) => {
    app.get('/api/zedcode/relay/status', async (_req, res) => {
      try {
    ...
    app.post('/api/zedcode/relay/enable', express.json({ limit: '16kb' }), async (req, res) => {
      try {
    ...
    app.post('/api/zedcode/relay/disable', async (_req, res) => {
      try {
        const current = await readConfig();
