...
plugins — this module replicates that mechanism as an ZedCode runtime API.
...
- `resolve.js` — model selection, mirroring OpenCode's `getSmallModel` chain:
  0. ZedCode's own settings override (Settings → Sessions → Small Model):
