...
describe('normalizeCustomOpenAIBaseURL', () => {
  const originalRuntime = process.env.ZEDCODE_RUNTIME;
  const originalAllowRemote = process.env.ZEDCODE_ALLOW_REMOTE_OPENAI_COMPAT_URLS;

  afterEach(() => {
    // Restore env vars after each test
    if (originalRuntime === undefined) {
      delete process.env.ZEDCODE_RUNTIME;
    } else {
      process.env.ZEDCODE_RUNTIME = originalRuntime;
    }
    if (originalAllowRemote === undefined) {
      delete process.env.ZEDCODE_ALLOW_REMOTE_OPENAI_COMPAT_URLS;
    } else {
      process.env.ZEDCODE_ALLOW_REMOTE_OPENAI_COMPAT_URLS = originalAllowRemote;
    }
  });

  it('rejects remote URLs when ZEDCODE_RUNTIME is not set (web)', () => {
    delete process.env.ZEDCODE_RUNTIME;
    delete process.env.ZEDCODE_ALLOW_REMOTE_OPENAI_COMPAT_URLS;
  ...
  it('allows remote URLs when ZEDCODE_RUNTIME is desktop', () => {
  ...
  it('allows remote URLs when ZEDCODE_ALLOW_REMOTE_OPENAI_COMPAT_URLS is true', () => {
    delete process.env.ZEDCODE_RUNTIME;
    process.env.ZEDCODE_ALLOW_REMOTE_OPENAI_COMPAT_URLS = 'true';
  ...
  it('denies remote URLs on desktop when env var is explicitly false', () => {
    process.env.ZEDCODE_RUNTIME = 'desktop';
    process.env.ZEDCODE_ALLOW_REMOTE_OPENAI_COMPAT_URLS = 'false';
