...
## Purpose
This module contains the ZedCode message-stream WebSocket protocol and runtime bridge. It keeps the browser-facing WebSocket transport separate from the upstream OpenCode SSE transport.
...
- ZedCode still fetches OpenCode upstream event streams over SSE.
...
- Global synthetic events such as `zedcode:session-status`, `zedcode:session-activity`, `zedcode:notification`, and `zedcode:heartbeat` are preserved on the WS path, but heartbeat frames are emitted only while an upstream SSE stream is actively attached.
