...
- App id / package: `com.zedcode.app`; app name `ZedCode`.
...
- **Deep links** — `zedcode://` URL scheme; a reusable intent vocabulary (`apps/deepLinks.ts`)
...
- **iOS widgets + Control Center + Notification Service Extension** — WidgetKit extension
  (`ZedCodeWidget`), a Control Center control, and an NSE (`ZedCodeNotificationService`)
  that refreshes widgets from push. All share the App Group `group.com.zedcode.app`.
...
- Extensions: `ZedCodeWidget` (WidgetKit, deployment 17.0) and `ZedCodeNotificationService`
  (NSE, 15.5), both hand-wired into `App.xcodeproj/project.pbxproj` and embedded via a copy phase.
- App Group `group.com.zedcode.app` in all three targets' entitlements (app + widget + NSE).
- `Info.plist`: `CFBundleURLTypes` scheme `zedcode`, `NSCameraUsageDescription`.
- Push entitlement (aps-environment) required.
- APNs `mutable-content: 1` (set server/relay side) wakes the NSE to refresh widgets.

### Android (`android/app`)

- `google-services.json` (committed; Firebase project `zedcode-8bf7e`). The Google Services
...
- Apple Developer account; App IDs for the app **and** both extensions
  (`com.zedcode.app`, `.ZedCodeWidget`, `.ZedCodeNotificationService`), each enabled for
