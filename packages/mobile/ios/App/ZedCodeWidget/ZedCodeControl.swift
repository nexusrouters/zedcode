...
// Control Center control (iOS 18+): tap the ZedCode logo to start a new session.
//
...
@available(iOS 18.0, *)
struct ZedCodeNewSessionControl: ControlWidget {
    var body: some ControlWidgetConfiguration {
        StaticControlConfiguration(kind: "ZedCodeNewSessionControl") {
        ...
        .description("Start a new ZedCode session.")
    }
}

@available(iOS 18.0, *)
struct OpenNewSessionIntent: AppIntent {
    static let title: LocalizedStringResource = "New ZedCode Session"
    static let openAppWhenRun: Bool = true
    static let isDiscoverable: Bool = true

    @MainActor
    func perform() async throws -> some IntentResult & OpensIntent {
        return .result(opensIntent: OpenURLIntent(URL(string: "zedcode://new")!))
    }
}
