...
enum WidgetStore {
    static let appGroup = "group.com.zedcode.app"
...
enum WidgetDeepLink {
    static func newSession() -> URL { URL(string: "zedcode://new")! }
    static func attention() -> URL { URL(string: "zedcode://sessions?filter=attention")! }
    static func status() -> URL { URL(string: "zedcode://status")! }
    static func settings() -> URL { URL(string: "zedcode://settings")! }
    static func changes() -> URL { URL(string: "zedcode://changes")! }
    static func files() -> URL { URL(string: "zedcode://view/files")! }
    static func instances() -> URL { URL(string: "zedcode://view/instances")! }
    static func session(_ id: String) -> URL {
        let encoded = id.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? id
        return URL(string: "zedcode://session/\(encoded)") ?? newSession()
    }
}
...
// MARK: - Logo (full ZedCode mark drawn from the SVG)

/// The ZedCode logo, drawn to match packages/web/public/logo-dark-512x512.svg: an
