# termigo-shell-integration (zshrc)
#
...
# zsh, so we shadow $? into `_termigo_ret`.

{
  _termigo_user_zdotdir="${TERMIGO_USER_ZDOTDIR:-$HOME}"
  [ -f "$_termigo_user_zdotdir/.zshrc" ] && source "$_termigo_user_zdotdir/.zshrc"
  unset _termigo_user_zdotdir
}

if [[ -n "$TERMIGO_CLI" && -x "$TERMIGO_CLI" ]]; then
  termigo() {
    command "$TERMIGO_CLI" "$@"
  }
fi
...
if [[ -z "$__TERMIGO_HOOKS_LOADED" ]]; then
  __TERMIGO_HOOKS_LOADED=1
  ...
  _termigo_precmd() {
    local _termigo_ret=$?
    printf '\e]133;D;%s\e\\' "$_termigo_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_termigo_urlencode "$PWD")"
  ...
  _termigo_preexec() {
    ...
    [[ -n "$TERMIGO_BLOCKS" ]] && _termigo_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _termigo_precmd
    add-zsh-hook preexec _termigo_preexec
  fi
