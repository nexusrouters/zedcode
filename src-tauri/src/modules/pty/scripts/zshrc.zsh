# zedcode-shell-integration (zshrc)
#
...
# zsh, so we shadow $? into `_zedcode_ret`.

{
  _zedcode_user_zdotdir="${ZEDCODE_USER_ZDOTDIR:-$HOME}"
  [ -f "$_zedcode_user_zdotdir/.zshrc" ] && source "$_zedcode_user_zdotdir/.zshrc"
  unset _zedcode_user_zdotdir
}

if [[ -n "$ZEDCODE_CLI" && -x "$ZEDCODE_CLI" ]]; then
  zedcode() {
    command "$ZEDCODE_CLI" "$@"
  }
fi
...
if [[ -z "$__ZEDCODE_HOOKS_LOADED" ]]; then
  __ZEDCODE_HOOKS_LOADED=1
  ...
  _zedcode_precmd() {
    local _zedcode_ret=$?
    printf '\e]133;D;%s\e\\' "$_zedcode_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_zedcode_urlencode "$PWD")"
  ...
  _zedcode_preexec() {
    ...
    [[ -n "$ZEDCODE_BLOCKS" ]] && _zedcode_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _zedcode_precmd
    add-zsh-hook preexec _zedcode_preexec
  fi
