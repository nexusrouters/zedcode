# zedcode-shell-integration (PowerShell)
...
if ($global:__ZEDCODE_HOOKS_LOADED) { return }
$global:__ZEDCODE_HOOKS_LOADED = $true

if ($env:ZEDCODE_CLI -and (Test-Path -LiteralPath $env:ZEDCODE_CLI -PathType Leaf)) {
    function global:zedcode {
        & $env:ZEDCODE_CLI @args
    }
}
...
if (Test-Path Function:prompt) {
    Copy-Item Function:prompt Function:__zedcode_user_prompt -Force -ErrorAction SilentlyContinue
}

function global:__zedcode_urlencode {
    param([string]$s)
...
function global:__zedcode_install_readline {
    if ($global:__zedcode_readline_done) { return }
    if (-not (Test-Path Function:PSConsoleHostReadLine)) { return }
    $global:__zedcode_readline_done = $true
    ...
    Copy-Item Function:PSConsoleHostReadLine Function:global:__zedcode_user_readline -Force
    function global:PSConsoleHostReadLine {
        try {
            $line = __zedcode_user_readline
        } catch {
            # Self-heal: restore the original reader so a broken wrapper can
            # never lock the shell out of input.
            Copy-Item Function:__zedcode_user_readline Function:global:PSConsoleHostReadLine -Force
            return ''
        }
        try {
            if ($line -is [string] -and $line.Trim().Length -gt 0) {
                $global:__zedcode_block_seen = $true
                $esc = [char]27
...
function global:prompt {
    ...
    if ($loc.Provider.Name -eq 'FileSystem') {
        ...
        $cwdEnc = __zedcode_urlencode $cwd
    ...
    if ($env:ZEDCODE_BLOCKS) {
        $global:LASTEXITCODE = $lec
        $gap = if ($global:__zedcode_block_seen) { "`n`n" } else { "`n" }
        return "$oscD$oscA$osc7$gap$oscB"
    }

    $original = if (Test-Path Function:__zedcode_user_prompt) {
        try { & __zedcode_user_prompt } catch { "PS $((Get-Location).Path)> " }
    } else {
