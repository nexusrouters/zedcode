...
///
/// Stages into `<dir>/.<filename>.zedcode.tmp`, fsyncs it, then renames over
...
/// Stage `bytes` into `<dir>/.<filename>.zedcode.tmp` (opened via `open_tmp`),
