...
///
/// Stages into `<dir>/.<filename>.termigo.tmp`, fsyncs it, then renames over
...
/// Stage `bytes` into `<dir>/.<filename>.termigo.tmp` (opened via `open_tmp`),
