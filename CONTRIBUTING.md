...
```bash
git clone https://github.com/zedcode/zedcode.git
cd zedcode
bun install
```
...
Both are configurable via env vars: `ZEDCODE_PORT`, `ZEDCODE_HMR_UI_PORT`, `ZEDCODE_HMR_API_PORT`.
...
`bun run vscode:dev` opens an Extension Development Host automatically. You can override the editor or workspace with `ZEDCODE_VSCODE_BIN` and `ZEDCODE_VSCODE_DEV_WORKSPACE`.

Example: `ZEDCODE_VSCODE_BIN=cursor bun run vscode:dev`.
...
| `bun run pack:web` | Create a package archive for `@zedcode/web` |
...
```bash
ZEDCODE_TARGET_ARCH=x64 bun run electron:build
# On an arm64 host:
ZEDCODE_TARGET_ARCH=arm64 bun run electron:build
...
Open an [issue](https://github.com/zedcode/zedcode/issues) or ask in [Discord](https://discord.gg/ZYRSdnwwKA).
