...
React components use `useRuntimeAPIs()` or `useRuntimeAPI()`. Non-React modules use `getRegisteredRuntimeAPIs()` only when hooks are impossible. Do not introduce direct reads of `window.__ZEDCODE_RUNTIME_APIS__` in feature code.

## VS Code Route Decisions

| Route type | VS Code behavior |
|---|---|
| ZedCode local route | Handle in the webview and bridge to extension host when needed |
...
Register explicit ZedCode handling before generic proxy fallback. Silent empty fallback is not parity.
