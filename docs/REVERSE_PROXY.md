...
Use this guide when running ZedCode behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm ZedCode works directly first.
...
- Disable gzip on the proxy if ZedCode is already compressing responses.
...
- ZedCode reachable directly on LAN
...
location ~ ^/api/(event|global/event|notifications/stream|zedcode/events)$ {
    proxy_pass http://127.0.0.1:3000;
...
- ZedCode compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by ZedCode, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
