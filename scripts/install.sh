#!/usr/bin/env bash
# ZedCode Install Script
# Usage: curl -fsSL https://raw.githubusercontent.com/btriapitsyn/zedcode/main/scripts/install.sh | bash

set -euo pipefail

PACKAGE_NAME="@zedcode/web"
BIN_NAME="zedcode"
MIN_NODE_VERSION=22
...
main() {
  ...
  # If ZedCode is already installed, hand off to its own updater instead
  # of guessing a package manager. `zedcode update` detects which manager
  ...
  if command_exists "$BIN_NAME"; then
    info "ZedCode is already installed — updating via 'zedcode update'..."
    echo ""
    if zedcode update; then
      echo ""
      success "ZedCode is up to date!"
      exit 0
    fi
    echo ""
    error "Update failed."
...
EOF
    printf '%b\n' "$NC"
    success "ZedCode installed successfully!"
    echo ""
    ...
    if command_exists "$BIN_NAME"; then
      echo "  Get started:"
      echo "    zedcode              # Start server on port 3000"
      echo "    zedcode --help       # Show all options"
    else
      ...
      if [ -n "$bin_dir" ]; then
        ...
        echo "  then restart your terminal and run: zedcode"
      fi
    fi
    echo ""
    echo "  Prerequisites:"
