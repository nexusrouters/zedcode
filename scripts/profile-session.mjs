#!/usr/bin/env node
/**
 * Fully automated streaming capture for ZedCode.
 *
 ...
 * prompt through the supported `zedcode session` CLI, and records until the
...
Records what ZedCode costs while an assistant response streams in.

Options:
  --url <url>              ZedCode URL (default: http://localhost:3000)
  --port <port>            ZedCode CLI port (default: from --url)
...
/**
 * Runs an `zedcode session` subcommand and returns its parsed JSON.
