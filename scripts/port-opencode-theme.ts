...
/*
 * Port OpenCode themes into full ZedCode theme JSON files.
 *
 * Usage:
 *   bun run themes:port:opencode --list
 *   bun run themes:port:opencode github cursor lucent-orng --force
 *   bun run themes:port:opencode aura --out-dir /tmp/zedcode-theme-port-test --force
 ...
 *     or existing ZedCode theme JSON files
 *   - writes complete ZedCode light/dark theme JSON output with solid core
...
type ThemeSource = DesktopTheme | ContextThemeJson | ZedCodeTheme;
...
function isZedCodeTheme(value: ThemeSource): value is ZedCodeTheme {
...
function syntheticTokensFromZedCodeTheme(theme: ZedCodeTheme): ResolvedTheme {
...
async function main(): Promise<void> {
  ...
  const generated: Array<{ fileName: string; theme: ZedCodeTheme }> = [];
    ...
    if (isZedCodeTheme(source)) {
      ...
      generated.push({
        ...
        theme: buildTheme(
          syntheticTheme,
          variant,
          syntheticTokensFromZedCodeTheme(source),
