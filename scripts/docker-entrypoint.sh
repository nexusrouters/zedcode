...
# alias; ZEDCODE_UI_PASSWORD is the canonical runtime variable.
if [ -z "${ZEDCODE_UI_PASSWORD:-}" ] && [ -n "${UI_PASSWORD:-}" ]; then
  ZEDCODE_UI_PASSWORD="$UI_PASSWORD"
  export ZEDCODE_UI_PASSWORD
fi

if [ -n "${ZEDCODE_UI_PASSWORD:-}" ]; then
...
ZEDCODE_HOST="${ZEDCODE_HOST:-0.0.0.0}"
export ZEDCODE_HOST
...
if [ -n "${ZEDCODE_UI_PASSWORD:-}" ]; then
  set -- "$@" --ui-password "$ZEDCODE_UI_PASSWORD"
fi
"$@"
