...
ZedCode provides shared web, desktop, VS Code, hosted-mobile, and native-mobile UI surfaces for OpenCode.
...
- `packages/web`: web surfaces, ZedCode server, managed/external OpenCode lifecycle, and CLI.
...
- `packages/mobile`: Capacitor iOS/Android shell; bundles the mobile web surface and connects to an existing ZedCode server.
- `packages/docs`: product documentation; not a Bun workspace.

Shared UI calls official OpenCode APIs through `@opencode-ai/sdk/v2`. ZedCode-owned capabilities use `RuntimeAPIs`, `runtimeFetch`, and shared browser/realtime transport helpers. Server-side upstream integrations may use their owning runtime modules.

Electron starts the ZedCode backend in-process, never as a sidecar. Development may load loopback/HMR UI; packaged builds load staged assets through `zedcode-ui://` while the loopback server remains the API backend. Keep domain backends in web/runtime modules unless behavior is inherently native.
...
|---|---|
| Source/dependency changes, exports or package contracts, build/generated assets, or module ownership | `zedcode-change-discipline` |
...
|---|---|
| Change scope, abstraction discipline, and validation risk | `zedcode-change-discipline` |
