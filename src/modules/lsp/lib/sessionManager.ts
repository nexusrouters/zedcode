...
import type { ZedCodeLspClient } from "./client";
...
async function createSession(
  ...
  const [{ TauriLspTransport }, { ZedCodeLspClient }] = await Promise.all([
    import("./transport"),
    import("./client"),
  ]);

  if (ZedCodeLspClient.hostPid === null) {
    ZedCodeLspClient.hostPid = await invoke<number>("lsp_host_pid").catch(
      () => null,
    );
  }
  ...
  const client = new ZedCodeLspClient({
    transport,
    rootUri,
