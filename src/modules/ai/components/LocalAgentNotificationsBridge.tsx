...
export function LocalAgentNotificationsBridge() {
  ...
  useEffect(() => {
    ...
    if (status === "awaiting-approval") {
      fire("attention", "ZedCode needs your approval", "Approve a tool to continue");
    } else if (status === "error") {
      fire("error", "ZedCode run failed", error ?? undefined);
    } else if (status === "idle" && isBusy(was)) {
      fire("finished", "ZedCode finished", "Your task is ready");
    }
  }, [status, error]);
