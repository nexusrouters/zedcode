...
import {
  isLoggedIn as isZedcodeLoggedIn,
  logoutZedcode,
  startDeviceLogin,
...
export function ModelsSection() {
  ...
  const isConfigured = (id: ProviderId): boolean => {
    if (id === "zedcode") return zedcodeLoggedIn;
  ...
  const removeProvider = (id: ProviderId) => {
    if (id === "zedcode") {
      void logoutZedcode().then(() => setZedcodeLoggedIn(false));
            ...
            {visibleProviders.map((p) =>
              p.id === "zedcode" ? (
                <ZedcodeCard
                  key={p.id}
                  provider={p}
                  loggedIn={zedcodeLoggedIn}
                  onLoggedInChange={setZedcodeLoggedIn}
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(
          res.message ||
            (res.reason === "denied"
              ? "Access denied — no active Coding Plan."
              : res.reason === "expired"
                ? "The login request expired. Try again."
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-border/60 bg-card/60 px-3 py-2.5">
      ...
      <p className="text-[11px] text-muted-foreground">
        Sign in with your ZedCode account. Models come from your Coding Plan —
        no API key needed.
      </p>

      {code ? (
        <div className="rounded-md border border-border/60 bg-muted/40 px-2.5 py-2 text-[11px]">
          <div>
          ...
          <button
            type="button"
            className="mt-1 text-[10.5px] text-primary underline"
            onClick={() =>
              void openUrl(code.verification_uri_complete || code.verification_uri)
            }
          >
